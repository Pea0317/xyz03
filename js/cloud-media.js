...
(function (global) {
    ...
    // 按文件头 magic bytes 嗅探真实 MIME。浏览器给本地文件的 type 经常是空串或
    // application/octet-stream（尤其是音频，.mp3/.wav/.m4a 在部分系统/浏览器上就是个空串）——
    // 如果直接拿它定扩展名和 Content-Type，签名上传后对象就是错误类型，下载回来 <audio> 也放不了。
    // 统一按魔数嗅探，图片/音频/视频一视同仁，嗅不出就返回 null 让上层保留原类型
    function _sniffMime(bytes) {
        ...
        if (bytes[0] === 0x52 && bytes[1] === 0x49 && bytes[2] === 0x46 && bytes[3] === 0x46) {       // RIFF...
            if (bytes.length >= 12 && bytes[8] === 0x57 && bytes[9] === 0x41 && bytes[10] === 0x56 && bytes[11] === 0x45) return 'audio/wav';   // ...WAVE
            if (bytes.length >= 12 && bytes[8] === 0x57 && bytes[9] === 0x45 && bytes[10] === 0x42 && bytes[11] === 0x50) return 'image/webp';  // ...WEBP
            return null;
        }
        // 音频
        if (bytes[0] === 0x49 && bytes[1] === 0x44 && bytes[2] === 0x33) return 'audio/mpeg';          // ID3 标签 → mp3
        if (bytes[0] === 0xFF && (bytes[1] & 0xE0) === 0xE0) return 'audio/mpeg';                     // MPEG 帧同步 → mp3/aac
        ...
        if (bytes[0] === 0x23 && bytes[1] === 0x21 && bytes[2] === 0x41 && bytes[3] === 0x4D) return 'audio/amr';    // #!AMR
        // 视频 / 容器（EBML = webm/mkv）
        if (bytes[0] === 0x1A && bytes[1] === 0x45 && bytes[2] === 0xDF && bytes[3] === 0xA3) return 'video/webm';
        // MP4 / M4A（iso box：offset 8 是 'ftyp'，offset 12 是 brand）
        if (bytes.length >= 16 && bytes[8] === 0x66 && bytes[9] === 0x74 && bytes[10] === 0x79 && bytes[11] === 0x70) {
            var brand = String.fromCharCode(bytes[12], bytes[13], bytes[14], bytes[15]);
            if (brand === 'M4A ' || brand === 'M4B ' || brand === 'M4P ') return 'audio/mp4';
            if (brand === 'isom' || brand === 'mp42' || brand === 'avc1' || brand === 'qt  ') return 'video/mp4';
            return 'video/mp4';
        }
        return null;
    }

    // 把 Blob 的类型修正常：类型明确（有真实 mime 且不是通用兜底）直接用；
    // 否则读文件头嗅探，改造成带正确 type 的新 Blob——这样算出来的扩展名、签进去的
    // Content-Type、以及下载后 <img>/<audio> 拿到的对象类型才都对，本地传的歌曲才能放
    async function _ensureMediaType(blob) {
        if (!blob) return blob;
        var t = blob.type || '';
        if (t && t !== 'application/octet-stream' && t !== 'text/plain' && t !== 'binary/octet-stream') {
            return blob;
        }
        try {
            var buf = await (blob.slice ? blob.slice(0, 32) : blob).arrayBuffer();
            var mime = _sniffMime(new Uint8Array(buf));
    ...
    async function uploadMedia(source, category, id) {
        ...
        blob = await _ensureMediaType(blob);
    ...
    async function fetchMediaUrl(ref) {
        ...
        var promise = (async function () {
            ...
            // 老对象可能存成了错误的类型（比如 octet-stream），下载时按文件头修正，
            // 否则 <audio>/<img> 拿到错误类型的 blob: URL 会直接放不出来
            blob = await _ensureMediaType(blob);
