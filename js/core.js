...
// 字卡分组的 id 保障：老备份/第三方内容包的 customReplyGroups 分组可能没有 id（或 id 不是合法数字），
// 而字卡库按分组筛选时用 parseInt 转数字后 find(g.id===…) 查找——无 id 就变成 NaN 匹配不到，
// 点开分组的筛选 chip 会显示"分组不存在"（L508/L620）。补一个唯一数字 id 即可（在乱序且不改变 items 的前提下）。
window.ensureReplyGroupIds = function (groups) {
    if (!Array.isArray(groups)) return groups;
    var base = Date.now();
    groups.forEach(function (g, i) {
        if (!g || typeof g !== 'object' || Array.isArray(g)) return;
        if (typeof g.id !== 'number' || !isFinite(g.id) || g.id <= 0) g.id = base + i;
    });
    return groups;
};
...
// 属性上下文专用转义（在 escapeHtml 基础上把反引号也转掉，防 onclick="/href=" 里用反引号包裹的突破）
function escAttr(s) {
    if (s == null) return '';
    return escapeHtml(s).replace(/`/g, '&#96;');
}

// 内联 onclick="/href=" 里单引号 JS 字符串参数的专用编码：
// 先做 HTML 属性级转义（& > < 以及终止属性值的双引号），再把 JS 字符串里的单引号转成 \u0027，
// 反斜杠转成 \\。保证值不会突破「单引号 JS 字符串」也不会突破「双引号 HTML 属性」。
function jsStr(s) {
...
// 安全合并：把外部 JSON/备份/云端数据合进运行时配置对象时，
// 显式拒绝 __proto__ / constructor / prototype 键，避免污染目标对象的原型链；
// 也绝不调用 Object.assign 那种会走 [[Set]] setter 的写法。
...
autoSendInterval: 5,
        ...
        freeSentenceEnabled: false,
        freeSentenceEmojiEnabled: false,
        timeFormat: 'HH:mm',
        customSoundUrl: '',
        ...
        function renderBackgroundGallery() {
            ...
            savedBackgrounds.forEach((bg, index) => {
                ...
                if (bg.type === 'image' || bg.type === 'gif') {
                    ...
                    item.innerHTML = `<img src="${escAttr(displaySrc)}" loading="lazy" alt="bg">`;
                } else {
                    const bgColor = /^[#()%0-9a-zA-Z.,\s\-]+$/.test(bg.value) ? bg.value : 'var(--secondary-bg)';
                    item.innerHTML = `<div class="bg-color-block" style="background: ${bgColor}"></div>`;
                }
...
const loadData = async () => {
    try {
        ...
        const results = await Promise.allSettled([
            ...
            localforage.getItem(getStorageKey('chatTransfer'))
        ]);
        ...
        const savedTransferData = getVal(23);

        if (savedTransferData && typeof savedTransferData === 'object') {
            window.transferData = savedTransferData;
        }

        if (savedPartnerPersonas) partnerPersonas = savedPartnerPersonas;

        if (savedSettings) safeAssign(settings, savedSettings);
        ...
        // 迁移：老格式导入的分组可能没有数字 id，点开分组会显示"分组不存在"，
        // 启动时统一补 id（只此一次，后续保存自动带 id）。
        if (Array.isArray(window.customReplyGroups) && window.customReplyGroups.length) {
            ensureReplyGroupIds(window.customReplyGroups);
        }
...
const saveData = async () => {
    ...
    const promises = [
        ...
        { key: 'chatTransfer',          val: () => localforage.setItem(getStorageKey('chatTransfer'), window.transferData || { myBalance: 100000, systemBalance: 100000, records: [] }) },
    ];
...
if (customIntros && customIntros.length > 0) {
    ...
    const _siteNameFallback = (settings && settings.siteName) || (function(){ try { return localStorage.getItem('siteName'); } catch(e){ return null; } })() || "传讯";
    document.getElementById('welcome-title-glitch').textContent = _siteNameFallback;
...
// ─── 保活机制：记录活跃时间 + 离线补发对方消息 ────────────────────────────
// 记录最后一次活跃时间（关闭/后台/定时心跳时写入），用于下次打开时计算离开时长
window._recordLastActive = function () {
    try { localStorage.setItem('partnerLastActiveTs', String(Date.now())); } catch (e) {}
};

// 用户重新打开网站时，补发离开期间对方「本该」发来的消息（自动发送开启时）
window._catchUpMissedMessages = function () {
    try {
        if (!settings.autoSendEnabled) return;
        const intervalMs = (settings.autoSendInterval || 5) * 60 * 1000;

        let lastTs = null;
        try { const raw = localStorage.getItem('partnerLastActiveTs'); if (raw) lastTs = parseInt(raw, 10); } catch (e) {}
        if (!lastTs) { window._recordLastActive(); return; }

        const awayMs = Date.now() - lastTs;
        if (awayMs < intervalMs) { window._recordLastActive(); return; }

        // 计算离开期间应产生的回复条数（封顶，避免一次性刷屏）
        let missed = Math.floor(awayMs / intervalMs);
        if (missed > 12) missed = 12;

        // 可用回复池（沿用 simulateReply 的禁用项过滤）
        const basePool = (customReplies || []).map(r => String(r || '').trim()).filter(Boolean);
        let disabled = new Set();
        try { const raw = localStorage.getItem('disabledReplyItems'); if (raw) disabled = new Set(JSON.parse(raw)); } catch (e) {}
        (window.customReplyGroups || []).forEach(g => { if (g.disabled && Array.isArray(g.items)) g.items.forEach(it => disabled.add(it)); });
        const replyPool = basePool.filter(r => !disabled.has(r));
        if (!replyPool.length) { window._recordLastActive(); return; }

        let added = 0;
        for (let i = 0; i < missed; i++) {
            const ts = new Date(lastTs + (i + 1) * intervalMs);
            const text = replyPool[Math.floor(Math.random() * replyPool.length)];
            messages.push({
                id: 'catchup_' + ts.getTime() + '_' + i,
        ...
        if (added > 0) {
            ...
            if (typeof showNotification === 'function') showNotification((settings.partnerName || '对方') + ' 在你离开时发来了 ' + added + ' 条消息', 'info', 5000);
        }
    } catch (e) {
        console.warn('[keepalive] 离线补发失败', e);
    }
    window._recordLastActive();
};
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    if (settings.inChatAvatarEnabled) {
        ...
        if (shouldHide) {
            ...
            if (groupMember.avatar) {
                avatarDiv.innerHTML = `<img src="${escapeHtml(groupMember.avatar)}" style="width:100%;height:100%;object-fit:cover;">`;
            } else {
                const initials = (groupMember.name || '?').charAt(0).toUpperCase();
                avatarDiv.innerHTML = `<div style="width:100%;height:100%;background:var(--accent-color);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:#fff;">${escapeHtml(initials)}</div>`;
            }
        } else {
    ...
    if (msg.replyTo) {
        ...
        messageHTML += `<div class="reply-indicator" data-reply-id="${escapeHtml(msg.replyTo.id || '')}" style="cursor:pointer;" onclick="scrollToQuotedMessage(this)"><span class="reply-indicator-sender">${escapeHtml(repliedSender)}</span><span class="reply-indicator-text">${escapeHtml(repliedText)}</span></div>`;
    }

    const isImageOnly = !msg.text && !!msg.image;
    const isRedPacket = msg.type === 'red-packet';
    let content = (!isRedPacket && msg.text) ? `<div>${escapeHtml(msg.text).replace(/\n/g, '<br>')}</div>` : '';
    if (isRedPacket) {
        content = (typeof window.renderRedPacketMessage === 'function')
            ? window.renderRedPacketMessage(msg)
            : '<div style="padding:10px;color:#c4453c;">红包消息</div>';
    }
    if (msg.image) {
        ...
        const imgSrcEsc = escapeHtml(String(msg.image));
        const imgAttrs = `class="message-image${isImageOnly ? ' message-image-only' : ''}" alt="图片" style="max-width:${isImageOnly ? '100px' : '100px'}; border-radius: 12px;${!isImageOnly ? ' margin-top: 6px;' : ''} cursor: pointer;" data-view-src="${imgSrcEsc}" onclick="viewImage(this.getAttribute('data-view-src'))"`;
        if (isCloudImg) {
            content += `<img data-lazy-cloud-ref="${imgSrcEsc}" ${imgAttrs}>`;
        } else if (isPendingImg) {
            // 用一个包裹层放"上传中"角标
            content += `<div class="message-image-pending-wrap" style="position:relative;display:inline-block;">`
                + `<img data-pending-ref="${imgSrcEsc}" ${imgAttrs}>`
            ...
            content += `<img src="${imgSrcEsc}" ${imgAttrs}>`;
        }
    }
    messageHTML += content;

    const messageDiv = document.createElement('div');
    if (isImageOnly || isRedPacket) {
    ...
    if (isRedPacket) {
        ...
        if (rpCard) {
            const rpId = rpCard.dataset.rpId ? String(rpCard.dataset.rpId) : String((msg.redPacket && msg.redPacket.id) || msg.id);
            rpCard.addEventListener('click', (e) => {
                e.stopPropagation();
    ...
    if (message.sender === 'user' && message.type === 'normal' && typeof window._onUserMessage === 'function') {
        ...
        function sendMessage(textOverride = null, type = 'normal') {
            ...
            const createMessage = (imgSrc = null) => {
                ...
                if (type === 'normal' && window.TaPhoneApp && typeof window.TaPhoneApp.tryCollectChat === 'function') {
                    try {
                        if (text && typeof text === 'string') window.TaPhoneApp.tryCollectChat(text, messageData.timestamp, messageData);
                    } catch (e) { console.warn('[ta-phone hook]', e); }
                }
                currentReplyTo = null;
                updateReplyPreview();
...
if (!isBatchMode && type === 'normal') {
    ...
    window._triggerDelayedReply(true);
        ...
        function updateBatchPreview() {
            ...
            if (batchMessages.length > 0) {
                listHTML = batchMessages.map((msg, index) => {
                    ...
                    if (msg.image) {
                        const isCloudImg = typeof msg.image === 'string' && msg.image.indexOf('oss://') === 0;
                        const imgEsc = escapeHtml(String(msg.image));
                        if (isCloudImg) {
                            preview = `<img data-lazy-cloud-ref="${imgEsc}" style="height:36px;width:36px;object-fit:cover;border-radius:6px;vertical-align:middle;margin-right:6px;">`;
                        } else {
                            preview = `<img src="${imgEsc}" style="height:36px;width:36px;object-fit:cover;border-radius:6px;vertical-align:middle;margin-right:6px;">`;
                        }
                    }
        ...
        window.simulateReply = function() {
            // ── 自由造句（聊天设置→节奏"自由造句"）：从字卡库的汉字中随机截取字/词拼成一句话 ──
            const _KAOMOJI_EMOJI = [
                '(｡•̀ᴗ-)✧', "( ˶'ᵕ'˶ )", '⊂(・▽・⊂)', '(๑¯◡¯๑)', '(*≧ω≦)',
                '(°▽°)', '(・ω<)★', '~(￣▽￣)~', '( •̀ ω •́ )✧', '♡( ◡‿◡ )',
                '✨', '💕', '🌙', '☕', '🌷', '🐾', '🍀', '🌸', '🌟', '💐'
            ];
            function _buildFreeSentence(pool) {
                if (!pool || !pool.length) return '';
                const hanChars = [];
                pool.forEach(t => {
                    const chars = Array.from(String(t).replace(/[^\u4e00-\u9fa5]/g, ''));
                    if (chars.length) hanChars.push(...chars);
                });
                if (!hanChars.length) return '';
                const pickToken = () => {
                    const len = 1 + Math.floor(Math.random() * 3); // 1~3 个字
                    const start = Math.floor(Math.random() * hanChars.length);
                    let s = '';
                    for (let k = 0; k < len; k++) s += hanChars[(start + k) % hanChars.length];
                    return s;
                };
                const parts = [];
                const n = 3 + Math.floor(Math.random() * 4); // 3~6 段
                for (let i = 0; i < n; i++) parts.push(pickToken());
                let text = parts.join(Math.random() < 0.5 ? '、' : '');
                if (Math.random() < 0.7) text += Math.random() < 0.5 ? '！' : Math.random() < 0.5 ? '？' : '……';
                return text;
            }
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            const recentUserMsgs = (settings.replyEnabled && !window._companionSilentTrigger)
                ...
                : [];
            window._surveyChatSurveySentThisBatch = false;
            for (let i = 0; i < replyCount; i++) {
                ...
                setTimeout(() => {
                    ...
                    let replyText = '';
                    let isSurveyQuestion = false;
                    // ── 聊天问卷：对方出题来自"问卷题库"，抽中概率和其他字卡完全一样 ──
                    //（把"一道问卷题"当成池子里的第 N+1 张卡：每张卡概率都是 1/(N+1)；
                    //  一整批回复里最多出一题，避免连着几条都是问卷刷屏）
                    try {
                        if (!window._surveyChatSurveySentThisBatch &&
                            typeof window._surveyChatReady === 'function' && window._surveyChatReady() &&
                            Math.random() < 1 / (replyPool.length + 1)) {
                            const sq = (typeof window._surveyChatQuestion === 'function') ? window._surveyChatQuestion() : null;
                            if (sq) {
                                replyText = String(sq).trim();
                                isSurveyQuestion = true;
                                window._surveyChatSurveySentThisBatch = true;
                            }
                        }
                    } catch (e) { isSurveyQuestion = false; }
                    if (!isSurveyQuestion) {
                    if (settings.freeSentenceEnabled && Math.random() < 0.5) {
                        // 自由造句：打乱字卡库汉字随机拼句，可再拼接颜文字/emoji
                        replyText = _buildFreeSentence(replyPoolOnce);
                        if (!replyText) {
                            // 字卡库没有可用汉字时回退普通抽取
                            for (let t = 0; t < 6; t++) {
                        ...
                        } else if (settings.freeSentenceEmojiEnabled && Math.random() < 0.5) {
                            const extra = _KAOMOJI_EMOJI[Math.floor(Math.random() * _KAOMOJI_EMOJI.length)];
                            replyText = Math.random() < 0.5
                                ? extra + ' ' + replyText
                                : replyText + ' ' + extra;
                        }
                    } else if (settings.combineReplyCards) {
                    ...
                    let finalText = isSurveyQuestion ? ('💭 ' + replyText) : replyText;
                    ...
                    if (i === replyCount - 1) {
                        ...
                        // 红包联动（仿 wz）：对方随机发红包 / 随机收取待领取红包 / 检查过期红包
                        try {
                        ...
                        } catch (e) { console.warn('[redPacket] 联动触发失败', e); }
                    }
                    } catch (e) {
        ...
        // 聊天问卷/其他功能需要"对方以聊天节奏说话"时的通用出口：
        // 复用主聊天的"正在输入"提示 + 消息音效 + 通知，观感跟 simulateReply 完全一致
        window._partnerTypingAndSay = function(text, opts) {
            opts = opts || {};
            const partnerName = settings.partnerName || '对方';
            const wait = opts.wait || 650;
            try {
                const tiW = document.getElementById('typing-indicator-wrapper');
                const tiL = document.getElementById('typing-indicator-label');
                if (tiL) tiL.textContent = partnerName + ' 正在输入';
            ...
            setTimeout(function () {
                ...
                } catch (e) { console.error('[partnerTypingAndSay] 消息插入失败', e); return; }
                try { playSound('message'); } catch (e) {}
                try { if (typeof window._sendPartnerNotification === 'function') window._sendPartnerNotification(partnerName, String(text == null ? '' : text)); } catch (e) {}
        ...
        async function viewImage(src) {
            ...
            modal.innerHTML = `
                <div style="position:relative;max-width:95vw;max-height:92vh;display:flex;align-items:center;justify-content:center;">
                    <img src="${escapeHtml(displaySrc)}" style="max-width:95vw;max-height:88vh;object-fit:contain;display:block;border-radius:8px;box-shadow:0 8px 40px rgba(0,0,0,0.6);" draggable="false">
                    ...
                    <a href="${escapeHtml(downloadHref)}" download style="position:fixed;bottom:24px;left:50%;transform:translateX(-50%);padding:10px 24px;background:rgba(255,255,255,0.15);border:1.5px solid rgba(255,255,255,0.3);border-radius:20px;color:#fff;font-size:13px;text-decoration:none;backdrop-filter:blur(8px);display:flex;align-items:center;gap:6px;"><i class="fas fa-download"></i> 保存图片</a>
                </div>`;
                    ...
                    if (_impConfirmBtn) _impConfirmBtn.onclick = function() {
                        ...
                        if (doSettings) {
                            if (importedData.settings) {
                                safeAssign(settings, importedData.settings);
                                try {
                                    if (settings.customFontUrl) applyCustomFont(settings.customFontUrl);
                                    if (settings.customBubbleCss) applyCustomBubbleCss(settings.customBubbleCss);
                                    if (settings.customGlobalCss) applyGlobalThemeCss(settings.customGlobalCss);
                        ...
                        if (doReplies  && importedData.customReplyGroups) window.customReplyGroups = ensureReplyGroupIds(importedData.customReplyGroups);
...
// 兜底：正常按 getItem 读取 sessionList 可能因瞬时失败/竞态返回空，
// 这里扫描 IndexedDB 里所有带 SESSION_ID 前缀的键，把真实残留的会话重建出来，
// 防止"读到空列表 → 当新用户 → 把会话列表整体覆盖成空"的连锁数据丢失
// （强制刷新、低端机、IndexedDB 锁竞争时最容易踩到）。
const _SESSION_KEYONLY_CACHE = new Set(['sessionList', 'lastSessionId', 'MIGRATION_V2_DONE', 'customThemes', 'themeSchemes', 'tour_seen', 'cloudSyncCfg', 'cloudSyncToken', 'cloudSyncStatus', 'cloudSyncLast', 'dailyGreetingShown', 'V2_DONE', 'seen']);

async function _recoverSessionsFromKeys() {
    try {
        const keys = await localforage.keys();
        const pfx = APP_PREFIX;
        const counts = {};
        keys.forEach(k => {
            ...
            if (us <= 0) return;
            const sid = rest.slice(0, us);
            const base = rest.slice(us + 1);
            if (!base || _SESSION_KEYONLY_CACHE.has(base) || _SESSION_KEYONLY_CACHE.has(sid) || sid.length < 6) return;
            counts[sid] = (counts[sid] || 0) + 1;
        });
        const sids = Object.keys(counts);
        if (!sids.length) return null;

        const sessions = [];
        for (const sid of sids) {
            let st = null;
            try { st = await localforage.getItem(pfx + sid + '_chatSettings'); } catch (e) {}
            sessions.push({
                id: sid,
                name: (st && (st.partnerName || st.myName)) || `会话 ${new Date(Number(sid) || Date.now()).toLocaleDateString()}`,
                createdAt: (st && st.createdAt) || Number(sid) || Date.now(),
                lastActive: (st && st.lastActive) || 0,
                _dataCount: counts[sid]
            });
        }
        sessions.sort((a, b) => (b._dataCount - a._dataCount) || (b.lastActive - a.lastActive) || (b.createdAt - a.createdAt));
        return sessions.map(s => ({ id: s.id, name: s.name, createdAt: s.createdAt, lastActive: s.lastActive }));
    } catch (e) {
        console.warn('[session] 从存储键重建会话列表失败:', e);
        return null;
    }
}
...
window.initializeSession = async function() {
    ...
    let sessionsData = await localforage.getItem(`${APP_PREFIX}sessionList`);
    sessionList = Array.isArray(sessionsData) ? sessionsData : [];

    // 会话列表异常为空时，不急于当"新用户"——先尝试从残留数据重建，
    // 只有确实没有任何历史数据时才 new 新会话
    if (!sessionList.length) {
        const rebuilt = await _recoverSessionsFromKeys();
        if (rebuilt && rebuilt.length) {
            console.warn(`[session] sessionList 为空，已从存储键重建 ${rebuilt.length} 个会话，防止数据被误覆盖`);
            sessionList = rebuilt;
            try { await localforage.setItem(`${APP_PREFIX}sessionList`, sessionList); } catch (e) {}
        }
    }
    ...
    // 自愈：解决"之前一次读取失败→新建了空会话"遗留的污染。
    // 若最终确定的当前会话名下没有任何真实数据(chatSettings/chatMessages 都没有)，
    // 但存储里还存在其它带了更多数据的会话，就把当前会话改认到数据最完整的那一个，
    // 并顺手修复 sessionList / lastSessionId，让用户已丢失的数据重新出现。
    try {
        const [curMsgs, curSet] = await Promise.all([
            localforage.getItem(`${APP_PREFIX}${SESSION_ID}_chatMessages`),
            localforage.getItem(`${APP_PREFIX}${SESSION_ID}_chatSettings`)
        ]);
        const curEmpty = (curSet === null || curSet === undefined) &&
            (curMsgs === null || curMsgs === undefined || (Array.isArray(curMsgs) && !curMsgs.length));
        if (curEmpty) {
            const recovered = await _recoverSessionsFromKeys();
            if (recovered && recovered.length && recovered[0].id !== SESSION_ID) {
                console.warn(`[session] 当前会话 ${SESSION_ID} 无数据，改认到数据最完整的会话 ${recovered[0].id}`);
                SESSION_ID = recovered[0].id;
                sessionList = recovered;
                try { await localforage.setItem(`${APP_PREFIX}sessionList`, sessionList); } catch (e) {}
            }
        }
    } catch (e) {
        console.warn('[session] 空会话自愈检查跳过:', e);
    }
