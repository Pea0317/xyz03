...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    const toggleSyncMap = {
        ...
        '#free-sentence-toggle': { prop: 'freeSentenceEnabled', name: '自由造句' },
        '#free-sentence-emoji-toggle': { prop: 'freeSentenceEmojiEnabled', name: '拼接颜文字/emoji' }
    };
...
// ── 自由造句（聊天设置→节奏）：开关 + 拼接颜文字/emoji 开关 ──
const freeSentenceToggle = document.getElementById('free-sentence-toggle');
const freeSentenceEmojiToggle = document.getElementById('free-sentence-emoji-toggle');

const updateFreeSentenceUI = () => {
    if (freeSentenceToggle) freeSentenceToggle.classList.toggle('active', !!settings.freeSentenceEnabled);
    if (freeSentenceEmojiToggle) freeSentenceEmojiToggle.classList.toggle('active', !!settings.freeSentenceEmojiEnabled);
};

updateFreeSentenceUI();

const savePillToggleNow = (label, on) => {
    if (typeof saveData === 'function') {
        try {
            const p = saveData();
            if (p && typeof p.catch === 'function') p.catch(e => console.error(`[${label}] 保存失败:`, e));
        } catch (e) { console.error(`[${label}] 保存失败:`, e); }
    }
    showNotification(`${label}已${on ? '开启' : '关闭'}`, 'success');
};

if (freeSentenceToggle) freeSentenceToggle.addEventListener('click', () => {
    settings.freeSentenceEnabled = !settings.freeSentenceEnabled;
    updateFreeSentenceUI();
    savePillToggleNow('自由造句', settings.freeSentenceEnabled);
});

if (freeSentenceEmojiToggle) freeSentenceEmojiToggle.addEventListener('click', () => {
    settings.freeSentenceEmojiEnabled = !settings.freeSentenceEmojiEnabled;
    updateFreeSentenceUI();
    savePillToggleNow('拼接颜文字/emoji', settings.freeSentenceEmojiEnabled);
});
...
const savedCover = safeGetItem(APP_PREFIX + 'playerCover');
    ...
    // 暴露给陪伴"一起听歌"复用同一份歌单（含用户自定义歌曲）
    window._playerSongList = songs;
    ...
    // 本地添加的歌（走云端存储的 oss://）万一放不出来，给个明确的提示而不是干等旋转——
    // 也顺便暴露"这首歌其实是依赖云端网络读取"这一点，方便排查
    audio.addEventListener('error', () => {
        const cur = songs[currentIndex];
        if (cur && cur.isCustom && typeof cur.url === 'string' && cur.url.indexOf('oss://') === 0) {
            if (typeof _markPlaying === 'function') _markPlaying(false);
            if (typeof showNotification === 'function') {
                showNotification('《' + cur.title + '》无法播放：本地歌曲由云端存储，请检查网络或云端配置是否正常', 'error');
            }
        }
    });
