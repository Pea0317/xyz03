...
(function () {
    ...
    ready(function init() {
        ...
        // ─────────── 视频通话按钮（点击弹出 语音/视频 选择窗） ───────────
        const videocallBtn = document.getElementById('videocall-btn');
        if (videocallBtn) {
            videocallBtn.addEventListener('click', () => {
                if (window.callFeature && typeof window.callFeature.openPicker === 'function') {
                    // 如果通话已在进行中，恢复窗口并提示
                    if (window.callFeature.isActive && window.callFeature.isActive()) {
                        ...
                        if (typeof showNotification === 'function') showNotification('正在进行通话', 'info');
                        return;
                    }
                    window.callFeature.openPicker();
                } else if (window.callFeature && typeof window.callFeature.startCall === 'function') {
                    if (window.callFeature.isActive && window.callFeature.isActive()) {
                        window.callFeature.restoreWindow && window.callFeature.restoreWindow();
                        if (typeof showNotification === 'function') showNotification('正在进行通话', 'info');
                        return;
                    ...
                    if (typeof showNotification === 'function') {
                        showNotification('通话功能未就绪', 'error');
                    }
                }
            });
        }
