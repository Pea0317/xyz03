...
(function() {
    ...
    function _load() {
        try {
            ...
            if (d) {
                ...
                if (typeof d.volume === 'number' && d.volume >= 0 && d.volume <= 1) t.volume = d.volume;
    ...
    function _audioEl() {
        ...
        if (typeof a.volume === 'number') a.volume = t.volume;
        return a;
    }
    ...
    function setVolume(pct) {
        ...
        t.volume = v;
        var a = _audioEl();
        if (typeof a.volume === 'number') a.volume = v;
        _save();
        renderVolume();
    }

    function renderVolume() {
        var inp = el('music-volume-input');
        if (inp) inp.value = Math.round(t.volume * 100);
        var pct = el('music-volume-pct');
        if (pct) pct.textContent = Math.round(t.volume * 100) + '%';
    }

    // 一起听时，对方偶尔帮忙调节音量（对方和用户都可以调节）
    function partnerAdjustVolume() {
        if (!t.together) return;
        var cur = Math.round(t.volume * 100);
        var delta = Math.round(Math.random() * 24 - 12);
        if (delta === 0) delta = 6;
        var next = Math.max(0, Math.min(100, cur + delta));
        setVolume(next);
        _log(_partnerName() + ' 把音量调到了 ' + next + '%');
        _notify(_partnerName() + ' 把音量调到了 ' + next + '%', 'info', 2500);
    }
    ...
    function renameSong(idx) {
        if (idx < 0 || idx >= t.playlist.length) return;
        var song = t.playlist[idx];
        var name = prompt('修改已添加歌曲名称：', song.title);
        if (name === null) return;
        name = name.replace(/^\s+|\s+$/g, '');
        if (!name) { _notify('歌曲名称不能为空', 'warning', 2000); return; }
        if (name.length > 60) name = name.slice(0, 60);
        song.title = name;
        _save();
        renderPlaylist();
        renderNowPlaying();
        _notify('歌曲名称已修改 ✓', 'success', 1800);
    }
    ...
    function renderPlaylist() {
        ...
        var html = t.playlist.map(function(s, idx) {
            ...
            return '<div class="mq-song ' + (active ? 'active' : '') + '" onclick="window.MusicApp && MusicApp.playIndex(' + idx + ')">'
                ...
                + '<span class="mq-song-edit" onclick="event.stopPropagation();window.MusicApp && MusicApp.renameSong(' + idx + ')" title="修改名称"><i class="fas fa-pen"></i></span>'
    ...
    window.MusicApp = {
        ...
        setVolume: setVolume,
        getVolume: function() { return t.volume; },
