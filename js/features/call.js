...
#call-picker.visible{display:flex;animation:cFi .3s ease;}
.call-picker-card{
    background:rgba(28,30,42,.94);
    border:1px solid rgba(255,255,255,.14);
    border-radius:22px;padding:26px 22px 12px;width:84%;max-width:340px;
    box-shadow:0 24px 80px rgba(0,0,0,.5);
    display:flex;flex-direction:column;gap:16px;
    animation:cCu .28s cubic-bezier(.34,1.56,.64,1);
}
.call-picker-title{font-size:16px;font-weight:700;color:#fff;text-align:center;}
.call-picker-sub{font-size:12px;color:rgba(255,255,255,.55);text-align:center;}
.call-picker-options{display:flex;gap:14px;}
.call-picker-opt{
...
.call-picker-opt:hover{background:rgba(255,255,255,.16);transform:translateY(-2px);}
.call-picker-opt:first-child i{color:#7ac86f;}
.call-picker-opt:last-child i{color:var(--accent-color,#e0698a);}
.call-picker-opt i{font-size:28px;}
.call-picker-opt .cp-name{font-size:14px;font-weight:600;}
.call-picker-cancel{
    padding:10px;border:none;border-radius:12px;background:none;
    color:rgba(255,255,255,.6);font-size:13px;cursor:pointer;font-family:inherit;
}
.call-picker-cancel:hover{color:#fff;background:rgba(255,255,255,.06);}
  ...
  <div id="call-window-inner">
    ...
    <div id="call-connecting-state">
      ...
      <div style="font-size:11px;color:rgba(255,255,255,.4);display:flex;align-items:center;gap:5px;">
        <i class="fas fa-video" id="call-conn-type-icon" style="font-size:9px;"></i> 正在连接
      </div>
...
<div id="call-picker">
  <div class="call-picker-card">
    <div class="call-picker-title">选择通话方式</div>
    <div class="call-picker-sub">邀请对方开始一段陪伴的时光吧</div>
    <div class="call-picker-options">
      <button class="call-picker-opt" id="call-pick-voice">
        <i class="fas fa-phone"></i>
        <span class="cp-name">语音通话</span>
      </button>
      <button class="call-picker-opt" id="call-pick-video">
        <i class="fas fa-video"></i>
        <span class="cp-name">视频通话</span>
      </button>
    </div>
    <button class="call-picker-cancel" id="call-picker-cancel">取消</button>
  </div>
</div>
        `;
    ...
    function applyCallKind() {
        const isVoice = S.kind === 'voice';
        const connIcon = document.getElementById('call-conn-type-icon');
        if (connIcon) connIcon.className = isVoice ? 'fas fa-phone' : 'fas fa-video';
        ['call-immersive-btn','call-bg-btn'].forEach(id => {
            const b = document.getElementById(id);
            if (b) b.style.display = isVoice ? 'none' : '';
        });
    }

    function sendCallMsg(dur) {
        if (dur < 2000) return;
        if (S.kind === 'voice') sendCallEvent('fa-phone', '语音通话已结束', fmt(dur));
        else sendCallEvent('fa-video', '视频通话已结束', fmt(dur));
    }

    // ─── 通话屏幕常亮（Screen Wake Lock API）─────────────────
    // 语音/视频通话期间保持屏幕常亮，不自动黑屏；挂断后自动恢复。
    let _wakeLock = null;
    function acquireWakeLock() {
        if (!('wakeLock' in navigator) || _wakeLock) return;
        try {
            navigator.wakeLock.request('screen').then(lock => {
                if (_wakeLock) { try { lock.release && lock.release(); } catch (e) {} return; }
                _wakeLock = lock;
                if (lock && typeof lock.addEventListener === 'function') {
                    lock.addEventListener('release', () => {
                        if (_wakeLock === lock) _wakeLock = null;
                        // 若因页面切换/隐藏被释放，回到页面且仍在通话中时重新获取
                        if (document.visibilityState === 'visible') syncWakeLock();
                    });
                }
                // 异步获取期间若通话已结束，立即释放，避免锁泄漏
                syncWakeLock();
            }).catch(e => console.warn('[call] wake lock 获取失败:', e));
        } catch (e) { console.warn('[call] wake lock 异常:', e); }
    }
    function releaseWakeLock() {
        if (_wakeLock) {
            try { const pr = _wakeLock.release(); if (pr && typeof pr.then === 'function') pr.catch(() => {}); } catch (e) {}
            _wakeLock = null;
        }
    }
    function syncWakeLock() {
        const inRing = document.getElementById('call-incoming-overlay')?.classList.contains('visible') === true;
        if (S.active || inRing) acquireWakeLock();
        else releaseWakeLock();
    }
    document.addEventListener('visibilitychange', syncWakeLock);

    function startCall(isPartner, kind) {
        if (!S.enabled) return;
        if (kind === 'voice' || kind === 'video') S.kind = kind;
    ...
    function showIncomingCall(kind) {
        ...
        const k = (kind === 'voice' || kind === 'video') ? kind : (Math.random() < 0.5 ? 'voice' : 'video');
        S.kind = k;
        const ov = document.getElementById('call-incoming-overlay');
        if (!ov) return;
        fillAv('call-inc-avatar'); fillNm('call-inc-name');
        const subText = document.getElementById('call-inc-sub-text');
        if (subText) subText.textContent = k === 'voice' ? '邀请您进行语音通话' : '邀请您进行视频通话';
        const connIcon = document.getElementById('call-conn-type-icon');
        if (connIcon) connIcon.className = k === 'voice' ? 'fas fa-phone' : 'fas fa-video';
        ...
        // 播放通话邀请音效
        try {
            if (typeof playSound === 'function') playSound(k === 'voice' ? 'invite_voice' : 'invite_videocall');
        ...
        try {
            if (typeof window._sendPartnerNotification === 'function') {
                ...
                const title = partnerName + (k === 'voice' ? ' 正在邀请你语音通话' : ' 正在邀请你视频通话');
                const body  = k === 'voice' ? '快接听吧 📞' : '快接听吧 📹';
                window._sendPartnerNotification(title, body);
            }
    ...
    function openPicker() {
        if (!S.enabled) return;
        if (S.active) { restoreWindow(); return; }
        document.getElementById('call-picker')?.classList.add('visible');
    }
    ...
    function bindEvents() {
        ...
        document.getElementById('call-inc-accept')?.addEventListener('click', () => {
            ...
            clearTimeout(S.incomingTimer);
            try { if (typeof window.stopCurrentSound === 'function') window.stopCurrentSound(); } catch(e) {}
            startCall(true, S.kind);
        });

        const _picker = document.getElementById('call-picker');
        document.getElementById('call-pick-voice')?.addEventListener('click', () => {
            _picker?.classList.remove('visible');
            startCall(false, 'voice');
        });
        document.getElementById('call-pick-video')?.addEventListener('click', () => {
            _picker?.classList.remove('visible');
            startCall(false, 'video');
        });
        document.getElementById('call-picker-cancel')?.addEventListener('click', () => _picker?.classList.remove('visible'));
        _picker?.addEventListener('click', (e) => { if (e.target === _picker) _picker.classList.remove('visible'); });
    ...
    window.callFeature = { startCall, endCall, showIncomingCall, restoreWindow, minimizeWindow, openPicker, isActive: () => S.active };
    ...
    window._callModule = {
        ...
        resumeFromSession: function(session) {
            ...
            try {
                ...
                ['call-inc-avatar','call-conn-avatar','call-win-avatar','call-mini-av'].forEach(fillAv);
                ['call-conn-name','call-win-name','call-mini-name'].forEach(fillNm);
                if (session.kind === 'voice' || session.kind === 'video') S.kind = session.kind;
                applyBg(); positionWindow();
                applyCallKind();
