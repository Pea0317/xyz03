 ...
 * 覆盖：数据模型（moyuRecords / moyuPartnerLocations / moyuActivities / currentMoyuRecord
...
(function () {
  ...
  function persist(keys) {
    (keys || []).forEach(function (k) {
      ...
      else if (k === 'moyuPartnerLocations') v = window.moyuPartnerLocations;
  ...
  async function loadMoyuData() {
    try {
      const [records, partnerLocations, activities, current, session, unread] = await Promise.all([
        localforage.getItem(KEY('moyuRecords')),
        localforage.getItem(KEY('moyuPartnerLocations')),
      ...
      if (Array.isArray(partnerLocations)) window.moyuPartnerLocations = partnerLocations;
    ...
    if (window.moyuPartnerLocations.length === 0) {
      window.moyuPartnerLocations = DEFAULT_LOCATIONS.slice();
      persist(['moyuPartnerLocations']);
    }
  ...
  window.openMoyuModal = function () {
    ...
    window.renderMoyuPartnerLocations();
    window.renderMoyuCards();
  ...
  // ==================== 标签页切换 ====================
  function setMoyuTabStyle(el, on) {
    if (!el) return;
    el.classList.toggle('active', !!on);
    el.style.background = on ? 'rgba(var(--accent-color-rgb), 0.16)' : 'transparent';
    el.style.color = on ? 'var(--accent-color)' : 'var(--text-secondary)';
  }
  ...
  window.switchMoyuTab = function (tab) {
    ...
    const cardsPanel = document.getElementById('moyu-cards-panel');
    const currentTab = document.getElementById('moyu-tab-current');
    const recordsTab = document.getElementById('moyu-tab-records');
    const cardsTab = document.getElementById('moyu-tab-cards');

    if (!currentPanel || !recordsPanel || !currentTab || !recordsTab) return;

    currentPanel.style.display = tab === 'current' ? 'block' : 'none';
    recordsPanel.style.display = tab === 'records' ? 'block' : 'none';
    if (cardsPanel) cardsPanel.style.display = tab === 'cards' ? 'block' : 'none';

    if (tab === 'current') {
      setMoyuTabStyle(currentTab, true);
      setMoyuTabStyle(recordsTab, false);
      setMoyuTabStyle(cardsTab, false);
    } else if (tab === 'records') {
      setMoyuTabStyle(currentTab, false);
      setMoyuTabStyle(recordsTab, true);
      setMoyuTabStyle(cardsTab, false);
    } else {
      setMoyuTabStyle(currentTab, false);
      setMoyuTabStyle(recordsTab, false);
      setMoyuTabStyle(cardsTab, true);
    }
  };
  ...
  window.renderMoyuStats = function () {
    ...
    const cardCountEl = document.getElementById('moyu-card-count');
    const locations = window.moyuPartnerLocations || [];
    ...
    if (cardCountEl) cardCountEl.textContent = (window.moyuActivities || []).length;
  };
  ...
  // ==================== 对方摸鱼地点板块（用户可给梦角添加专属地点） ====================
  window.renderMoyuPartnerLocations = function () {
    const listEl = document.getElementById('moyu-partner-locations-list');
    if (!listEl) return;
    ...
    if (locations.length === 0) {
      listEl.innerHTML = `
        <div style="text-align: center; padding: 20px 12px; color: var(--text-secondary);">
          ...
          <div style="font-size: 12px;">还没有给对方添加地点~</div>
        </div>
      `;
      return;
    }
    ...
    listEl.innerHTML = locations.map((loc, index) => `
      <div style="display: flex; justify-content: space-between; align-items: center; padding: 10px 12px; background: var(--secondary-bg); border-radius: 10px; margin-bottom: 6px; border: 1px solid var(--border-color);">
        <div style="display: flex; align-items: center; gap: 8px; min-width: 0;">
          ...
          <span style="font-size: 13px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${window.escapeHtml(loc)}</span>
        </div>
        <button onclick="window.removeMoyuPartnerLocation(${index})" style="background: none; border: none; color: #ff6b6b; cursor: pointer; padding: 4px 8px; font-size: 12px;">
  ...
  window.addMoyuPartnerLocation = function () {
    const input = document.getElementById('moyu-new-partner-location-input');
    if (!input) return;
    ...
    if (!window.moyuPartnerLocations) window.moyuPartnerLocations = [];
    if (window.moyuPartnerLocations.indexOf(name) !== -1) {
      if (typeof showNotification === 'function') showNotification('对方已在这个地点出没，换一个吧', 'error');
      return;
    }

    window.moyuPartnerLocations.push(name);
    persist(['moyuPartnerLocations']);
    if (typeof throttledSaveData === 'function') throttledSaveData();
    ...
    if (typeof showNotification === 'function') showNotification('对方摸鱼地点已添加', 'success');
  };

  window.removeMoyuPartnerLocation = function (index) {
    if (!confirm('确定要删除这个地点吗？')) return;

    if (window.moyuPartnerLocations && index >= 0 && index < window.moyuPartnerLocations.length) {
      window.moyuPartnerLocations.splice(index, 1);
      persist(['moyuPartnerLocations']);
      if (typeof throttledSaveData === 'function') throttledSaveData();
      window.renderMoyuPartnerLocations();
      window.renderMoyuStats();
      if (typeof showNotification === 'function') showNotification('对方地点已删除', 'success');
    }
  };

  // ==================== 字卡板块（摸鱼内容，用户可自加） ====================
  window.renderMoyuCards = function () {
    const listEl = document.getElementById('moyu-card-list');
    if (!listEl) return;

    const cards = window.moyuActivities || [];

    if (cards.length === 0) {
      listEl.innerHTML = `
        <div style="text-align: center; padding: 18px 12px; color: var(--text-secondary);">
          <i class="fas fa-layer-group" style="font-size: 24px; margin-bottom: 6px; opacity: 0.3; display: block;"></i>
          <div style="font-size: 12px;">还没有字卡，添加一张吧~</div>
        </div>
      `;
      return;
    }

    listEl.innerHTML = cards.map((txt, index) => `
      <div style="display: flex; align-items: center; gap: 8px; padding: 9px 12px; background: var(--primary-bg); border-radius: 10px; margin-bottom: 6px; border: 1px solid var(--border-color);">
        <i class="fas fa-fish" style="background: rgba(var(--accent-color-rgb), 0.12); color: var(--accent-color); width: 26px; height: 26px; display: flex; align-items: center; justify-content: center; border-radius: 8px; font-size: 11px; flex-shrink: 0;"></i>
        <span style="flex: 1; font-size: 13px; color: var(--text-primary); line-height: 1.4;">${window.escapeHtml(txt)}</span>
        <button onclick="window.removeMoyuCard(${index})" title="删除字卡" style="background: none; border: none; color: #ff6b6b; cursor: pointer; padding: 4px 6px; font-size: 12px;">
          <i class="fas fa-times"></i>
        </button>
      </div>
    `).join('');
  ...
  window.addMoyuCard = function () {
    const input = document.getElementById('moyu-new-card-input');
    if (!input) return;

    const name = input.value.trim();
    if (!name) {
      if (typeof showNotification === 'function') showNotification('请输入摸鱼内容', 'error');
      return;
    }

    if (!window.moyuActivities) window.moyuActivities = [];
    if (window.moyuActivities.indexOf(name) !== -1) {
      if (typeof showNotification === 'function') showNotification('该字卡已存在', 'error');
      return;
    }

    window.moyuActivities.push(name);
    persist(['moyuActivities']);
    if (typeof throttledSaveData === 'function') throttledSaveData();
    ...
    if (typeof showNotification === 'function') showNotification('字卡添加成功', 'success');
  };

  window.removeMoyuCard = function (index) {
    if (!confirm('确定要删除这张字卡吗？')) return;

    if (window.moyuActivities && index >= 0 && index < window.moyuActivities.length) {
      window.moyuActivities.splice(index, 1);
      persist(['moyuActivities']);
      if (typeof throttledSaveData === 'function') throttledSaveData();
      window.renderMoyuStats();
      window.renderMoyuCards();
      if (typeof showNotification === 'function') showNotification('字卡已删除', 'success');
    }
  };
  ...
  function init() {
    ...
    const addPartnerBtn = document.getElementById('moyu-add-partner-location-btn');
    if (addPartnerBtn) addPartnerBtn.addEventListener('click', window.addMoyuPartnerLocation);
    const partnerInput = document.getElementById('moyu-new-partner-location-input');
    if (partnerInput) {
      partnerInput.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') window.addMoyuPartnerLocation();
      });
    }

    const addCardBtn = document.getElementById('moyu-add-card-btn');
    if (addCardBtn) addCardBtn.addEventListener('click', window.addMoyuCard);
    const cardInput = document.getElementById('moyu-new-card-input');
    if (cardInput) {
      cardInput.addEventListener('keydown', function (e) {
        if (e.key === 'Enter') window.addMoyuCard();
      });
    }
  ...
  window.MoyuApp = {
    ...
    renderMoyuPartnerLocations: window.renderMoyuPartnerLocations,
