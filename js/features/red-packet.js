/**
 * red-packet.js - 红包功能模块（由 D:\wz 项目移植至 wz2）
 * 集成到聊天项目，使用 transferData 全局状态；转账记录随 chatTransfer 持久化。
 */

// wz2 环境没有 wz 那些全局转账模块，这里声明 transferData 全局变量供本模块使用
...
(function () {
    ...
    window.showRedPacketReceiveModal = function (recordId) {
        ...
        var titleText = isReturned ? '已过期' : escHtml(record.message);
        ...
        var html =
            '<div id="rp-receive-panel" style="text-align:center;position:relative;overflow:hidden;border-radius:16px;width:260px;min-height:380px;' + panelBg + 'display:flex;flex-direction:column;">' +
                ...
                '<div style="padding:30px 16px 20px;display:flex;flex-direction:column;align-items:center;flex:1;justify-content:center;">' +
                    ...
                    '</div>' +
                    '<div style="font-size:13px;color:rgba(255,255,255,0.9);margin-bottom:6px;">' + escHtml(senderName) + ' 发来的红包</div>' +
            ...
            openBtn.onclick = function () {
                ...
                panel.innerHTML =
                    ...
                    '<div style="padding:30px 16px 20px;display:flex;flex-direction:column;align-items:center;flex:1;justify-content:center;">' +
                        ...
                        '</div>' +
                        '<div style="font-size:13px;color:rgba(255,255,255,0.9);margin-bottom:6px;">' + escHtml(senderName) + ' 发来的红包</div>' +
                        '<div style="font-size:18px;font-weight:700;color:#ffd700;">' + escHtml(record.message) + '</div>' +
            ...
            '<div class="red-packet-card' + (isOpened ? ' opened' : '') + '" data-rp-id="' + recordId + '" style="width:260px;border-radius:6px;overflow:hidden;cursor:pointer;transition:transform 0.15s;position:relative;">' +
                ...
                '<div class="rp-body" style="' + bodyBg + 'padding:12px 14px 14px;color:#fff;position:relative;display:flex;align-items:center;gap:12px;">' +
                    ...
                    '<div class="rp-content" style="flex:1;min-width:0;">' +
                        ...
                        '<div class="rp-msg-text" style="font-size:11px;opacity:0.8;margin-top:2px;">' + escHtml(message) + '</div>' +
                    '</div>' +
                '</div>' +
                // 底部白色区域
    ...
    // ========== 红包来往记录（消息统计内查看）==========
    window.renderRedPacketRecords = function (container) {
        window.initTransferData();
        if (!container) return;

        var records = (transferData && Array.isArray(transferData.records)) ? transferData.records.slice().reverse() : [];
        var myName = getMyName();
        var partnerName = getPartnerName();

        var totalSent = 0, totalReceived = 0, sentCnt = 0, recvCnt = 0;
        records.forEach(function (r) {
            if (r.from === 'me') { totalSent += (r.amount || 0); sentCnt++; }
            else if (r.status === 'received') { totalReceived += (r.amount || 0); recvCnt++; }
        });
        ...
        var netColor = net >= 0 ? '#2ed573' : '#c4453c';
        var netText = (net < 0 ? '-' : '') + '&yen;' + fmt(Math.abs(net));

        var summaryHtml =
            '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:14px;">' +
              '<div style="background:var(--secondary-bg,#f5f5f5);border-radius:12px;padding:14px 10px;text-align:center;">' +
                '<div style="font-size:11px;color:var(--text-secondary,#888);margin-bottom:4px;">我发出</div>' +
                '<div style="font-size:18px;font-weight:700;color:#c4453c;">&yen;' + fmt(totalSent) + '</div>' +
                '<div style="font-size:11px;color:var(--text-secondary,#aaa);margin-top:2px;">' + sentCnt + ' 笔</div>' +
              '</div>' +
              '<div style="background:var(--secondary-bg,#f5f5f5);border-radius:12px;padding:14px 10px;text-align:center;">' +
                '<div style="font-size:11px;color:var(--text-secondary,#888);margin-bottom:4px;">我收到</div>' +
                '<div style="font-size:18px;font-weight:700;color:#2ed573;">&yen;' + fmt(totalReceived) + '</div>' +
                '<div style="font-size:11px;color:var(--text-secondary,#aaa);margin-top:2px;">' + recvCnt + ' 笔</div>' +
              '</div>' +
              '<div style="background:var(--secondary-bg,#f5f5f5);border-radius:12px;padding:14px 10px;text-align:center;">' +
                '<div style="font-size:11px;color:var(--text-secondary,#888);margin-bottom:4px;">净往来</div>' +
                '<div style="font-size:18px;font-weight:700;color:' + netColor + ';">' + netText + '</div>' +
                '<div style="font-size:11px;color:var(--text-secondary,#aaa);margin-top:2px;">收到 - 发出</div>' +
              '</div>' +
            '</div>';

        var listHtml = '';
        if (records.length === 0) {
            listHtml = '<div style="text-align:center;padding:32px 0;color:var(--text-secondary,#999);"><i class="fas fa-envelope-open-text" style="font-size:34px;display:block;margin-bottom:10px;opacity:0.5;"></i>还没有红包来往记录</div>';
        } else {
            listHtml = '<div style="display:flex;flex-direction:column;gap:8px;">';
            records.forEach(function (r) {
                var isSent = r.from === 'me';
                var who = isSent ? (myName + ' → ' + partnerName) : (partnerName + ' → ' + myName);
                var statusTxt = r.status === 'received' ? '已领取' : (r.status === 'returned' ? '已退回' : '待领取');
                var statusColor = r.status === 'received' ? '#2ed573' : (r.status === 'returned' ? '#bbb' : '#c4453c');
                var amountColor = isSent ? '#c4453c' : '#2ed573';
                listHtml +=
                    '<div style="display:flex;align-items:center;gap:12px;background:var(--secondary-bg,#f5f5f5);border-radius:12px;padding:12px 14px;">' +
                        '<div style="width:38px;height:38px;flex-shrink:0;border-radius:50%;background:' + (isSent ? '#c4453c' : '#2ed573') + ';display:flex;align-items:center;justify-content:center;color:#fff;">' +
                            '<i class="fas ' + (isSent ? 'fa-arrow-up' : 'fa-arrow-down') + '" style="font-size:14px;"></i>' +
                        '</div>' +
                        '<div style="flex:1;min-width:0;">' +
                            '<div style="font-size:13px;font-weight:600;color:var(--text-primary,#1a1a1a);display:flex;align-items:center;gap:6px;flex-wrap:wrap;">' +
                                '<span>' + esc(who) + '</span>' +
                                '<span style="font-size:11px;padding:2px 8px;border-radius:8px;color:' + statusColor + ';background:rgba(0,0,0,0.05);">' + statusTxt + '</span>' +
                            '</div>' +
                            '<div style="font-size:11px;color:var(--text-secondary,#999);margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + esc(r.message || '恭喜发财') + '</div>' +
                        '</div>' +
                        '<div style="text-align:right;flex-shrink:0;">' +
                            '<div style="font-size:16px;font-weight:700;color:' + amountColor + ';">' + (isSent ? '-' : '+') + '&yen;' + fmt(r.amount || 0) + '</div>' +
                            '<div style="font-size:10px;color:var(--text-secondary,#bbb);margin-top:2px;">' + fmtTime(r.createdAt) + '</div>' +
                        '</div>' +
                    '</div>';
            });
            listHtml += '</div>';
