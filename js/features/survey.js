...
(function () {
    ...
    // 从聊天输入框"表情→问卷"打开问卷页面时为 true：此时发出的问卷会走"聊天节奏回答"
    // （channel:'chat'，回答时间对齐对方的聊天回复速度，答案以"对方回复"的形态出现在聊天里）
    var _chatCtx = false;
        ...
        if (q.optKind === 'image') {
            ...
            if (opt.value) {
                if (window.CloudMedia && window.CloudMedia.isCloudRef && window.CloudMedia.isCloudRef(opt.value)) {
                    ...
                    thumb.innerHTML = '<img src="' + (typeof escAttr === 'function' ? escAttr(opt.value) : String(opt.value).replace(/"/g, '&quot;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/&/g, '&amp;')) + '">';
                }
            } else {
    ...
    function _submitCreate() {
        ...
        if (_editingSurveyId) {
            ...
            var survey = {
                id: _uid('sv'),
                createdAt: Date.now(),
                dueAt: _chatCtx ? _randomChatDueAt() : _randomDueAt(),
                status: 'pending',
                answeredAt: null,
                deletedAt: null,
                viewed: true,
            ...
            if (_chatCtx) survey.channel = 'chat';
            ...
            _refreshOpenViews();
            if (_chatCtx) {
                // 聊天上下文：题目先出现在聊天里，回答时间对齐聊天回复速度，
                // 到点后答案也会以"对方回复"的形态投递回聊天（单选回选项 / 多选拼成一句话）
                _sendChatSurveyQuestionMsg(survey);
                var chatWait = Math.max(0, survey.dueAt - Date.now()) + 150;
                setTimeout(function () { _answerChatSurveyById(survey.id); }, chatWait);
                if (typeof showNotification === 'function') showNotification('问卷已发出，对方会在聊天里回答你', 'success');
            } else {
    ...
    //    （聊天里发的问卷走同一个到点链路：to 点先算答案，再把答案以"对方回复"投递到聊天里）
    function _checkDueSurveys() {
        if (!_loaded) return;
        var now = Date.now();
        _data.askPartner.forEach(function (s) {
            if (s.status === 'pending' && !s.deletedAt && s.dueAt && now >= s.dueAt) _answerSurvey(s);
        });
    }

    // 真正的"算答案"一步：状态/角标/存储一次搞定，且只在 status==='pending' 时生效，
    // 所以"聊天定时器先到"和"60秒轮询后到"两种触发不会重复处理同一份问卷
    function _answerSurvey(s) {
        if (!s || s.status !== 'pending' || s.deletedAt) return;
        var selections = {};
        ...
        _save();
        _queueNotify({ type: 'answered', survey: s });
        _refreshOpenViews();
        if (s.channel === 'chat') _chatDeliverPartnerAnswer(s);
    }

    function _answerChatSurveyById(id) {
        var s = _data.askPartner.find(function (x) { return x.id === id; });
        if (s) _answerSurvey(s);
    }

    // 聊天里用的回答时间：完全对齐"对方的聊天回复速度"（节奏 tab 那组毫秒滑块），
    // 跟高级页那套 1~24 小时的问卷倒计时不是一回事
    function _randomChatDueAt() {
        var min = (typeof settings !== 'undefined' && settings.replyDelayMin) ? settings.replyDelayMin : 3000;
        var max = (typeof settings !== 'undefined' && settings.replyDelayMax) ? settings.replyDelayMax : 7000;
        if (max < min) max = min;
        return Date.now() + min + Math.random() * (max - min);
    }

    // 对方答完后，把答案拼成聊天回复——单选直接回选项，多选把选项拼成一句话
    function _surveyAnswerText(s) {
        var lines = [];
        (s.questions || []).forEach(function (q) {
            var sel = (s.selections && s.selections[q.id]) || [];
            var picked = (q.options || []).filter(function (o) { return sel.indexOf(o.id) !== -1; });
            if (!picked.length) return;
            var labels = picked.map(function (o) { return o.kind === 'image' ? '〔图片选项〕' : o.value; });
            if (q.type === 'multi') lines.push('「' + q.text + '」我的选择：' + labels.join('、'));
            else lines.push('「' + q.text + '」我选：' + labels[0]);
        });
        return lines.join('\n');
    }

    function _chatDeliverPartnerAnswer(s) {
        var text = _surveyAnswerText(s);
        if (!text) return;
        var msg = '🌟 你出的问卷我答好啦：\n' + text;
        try {
            if (typeof window._partnerTypingAndSay === 'function') {
                window._partnerTypingAndSay(msg);
                return;
            }
        } catch (e) {}
        try {
            addMessage({ id: Date.now() + Math.floor(Math.random() * 1000), sender: _partnerName(), text: msg, timestamp: new Date(), status: 'received', favorited: false, note: null, replyTo: null, type: 'normal' });
        } catch (e) { console.warn('[survey] 聊天回答消息发送失败', e); }
    }

    // 用户从聊天里出题时，先把题目以"我的消息"形态放进聊天，让对方知道被提问了
    function _sendChatSurveyQuestionMsg(s) {
        try {
            var lines = (s.questions || []).map(function (q, i) { return 'Q' + (i + 1) + ' ' + q.text; });
            var text = '📝 我来考考你～\n' + lines.join('\n');
            addMessage({ id: Date.now() + Math.floor(Math.random() * 1000), sender: 'user', text: text, timestamp: new Date(), status: 'sent', favorited: false, note: null, type: 'normal' });
            try { if (typeof window.playSound === 'function') window.playSound('send'); } catch (e) {}
        } catch (e) { console.warn('[survey] 聊天问卷消息发送失败', e); }
    }
    ...
    function _submitAskMeAnswers(id, answersMap) {
        ...
        if (!b) return;
        var wasChatAsk = (b.channel === 'chat' && b.status === 'sent');
        ...
        if (wasChatAsk) _chatEchoUserAnswer(b);
    }

    // 聊天里对方问的题，用户在问卷页答完后，把答案回显为一条聊天消息（双方都能看到）
    function _chatEchoUserAnswer(b) {
        try {
            var lines = (b.questions || []).map(function (q) {
                var a = (b.answers && b.answers[q.id]) || '';
                return '「' + q.text + '」' + (a ? a : '（未回答）');
            });
            var text = '我回答啦～\n' + lines.join('\n');
            addMessage({ id: Date.now() + Math.floor(Math.random() * 1000), sender: 'user', text: text, timestamp: new Date(), status: 'sent', favorited: false, note: null, type: 'normal' });
            try { if (typeof window.playSound === 'function') window.playSound('send'); } catch (e) {}
        } catch (e) { console.warn('[survey] 聊天回答回显失败', e); }
    }

    // ── 聊天里对方"出问卷"：跟 simulateReply 联动 ──────────────
    // 池子里有可用题目 + 没有还没答的聊天提问（避免连续刷屏）才参与抽签；
    // 抽中概率由主聊天的自动回复引擎按"和其他字卡一样"的 1/(N+1) 来定
    function _surveyChatReady() {
        if (!_loaded || !_data.askMe || !_data.bank || !_data.bank.length) return false;
        if (_data.askMe.some(function (x) { return x.status === 'sent' && x.channel === 'chat'; })) return false;
        return _data.bank.some(function (q) {
            if (q.hidden || !q.text) return false;
            if (q.groupId) {
                var g = (_data.bankGroups || []).find(function (x) { return x.id === q.groupId; });
                if (g && g.disabled) return false;
            }
            return true;
        });
    }

    // 从问卷题库里按 drawCount 公平抽一道题：落一条"梦角问我"记录（channel:'chat'），
    // 返回题干文本，由主聊天引擎拼成一条聊天消息发给用户
    function _surveyChatQuestion() {
        if (!_loaded || !_data.bank) return null;
        var pool = _data.bank.filter(function (q) {
            if (q.hidden || !q.text) return false;
            if (q.groupId) {
                var g = (_data.bankGroups || []).find(function (x) { return x.id === q.groupId; });
        ...
        pool.forEach(function (q) { if (typeof q.drawCount !== 'number') q.drawCount = 0; }); // 兼容老数据
        var min = null;
        pool.forEach(function (q) { if (min === null || q.drawCount < min) min = q.drawCount; });
        var tier = pool.filter(function (q) { return q.drawCount === min; });
        var picked = tier[Math.floor(Math.random() * tier.length)];
        picked.drawCount = (picked.drawCount || 0) + 1;
        var batch = {
            id: _uid('am'),
            sentAt: Date.now(),
            status: 'sent',
            channel: 'chat',
            viewed: false, // 角标先显示"new"，用户点开详情页之后才变"未回答"
            favorited: false,
            answeredAt: null,
            receiveDueAt: null,
            receivedAt: null,
            deletedAt: null,
            answers: null,
            questions: [{ id: 'bq_' + Date.now() + '_' + Math.floor(Math.random() * 999), text: picked.text }]
        };
        _data.askMe.push(batch);
        _save();
        _refreshOpenViews();
    ...
    window._surveyOpenListModal = function () { _chatCtx = false; _openListModal(); };
    // 聊天输入框"表情 → 问卷"入口：打开的是同一套"高级功能页面的问卷页面"，
    // 但标记聊天上下文，发出的问卷会走聊天链路（channel:'chat'）
    window._surveyOpenListModalChat = function () { _chatCtx = true; _openListModal(); };
    window._surveyChatReady = _surveyChatReady;
    window._surveyChatQuestion = _surveyChatQuestion;
