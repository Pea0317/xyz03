...
(function() {
    ...
    const _SK_PREFIX = (window.APP_PREFIX || '');
    const STORAGE_KEYS = {
        products: _SK_PREFIX + 'shop_products',
        cart: _SK_PREFIX + 'shop_cart',
        orders: _SK_PREFIX + 'shop_orders',
        balance: _SK_PREFIX + 'shop_balance',
        searchHistory: _SK_PREFIX + 'shop_search_history',
        giftCabinet: _SK_PREFIX + 'shop_gift_cabinet'
    };
    ...
    function showShop(tab) {
        ...
        const wrapper = document.getElementById('shop-modal');
        if (wrapper) {
    ...
    function hideShop() {
        ...
        if (wrapper) {
            ...
            if (wrapper._hideTimeout) clearTimeout(wrapper._hideTimeout);
            wrapper._hideTimeout = setTimeout(() => { wrapper.style.display = 'none'; }, 300);
        }
    }
    ...
    function switchTab(tab) {
        ...
        document.querySelectorAll('.shop-tab').forEach(t => t.classList.toggle('active', t.getAttribute('data-tab') === tab));
    ...
    function createProductCard(product) {
        const tags = (product.tags || []).split(',').filter(t => t.trim()).map(t =>
            `<span class="product-tag">${shopEsc(t.trim())}</span>`
        ).join('');
        const imgHtml = product.img
            ? `<img src="${shopEsc(product.img)}" style="width:100%;height:100%;object-fit:cover;">`
            : (shopEsc(product.icon) || '📦');
        return `
            <div class="product-card" data-id="${shopEsc(product.id)}" data-name="${shopEsc(product.name)}" data-tags="${shopEsc(product.tags)}" onclick="window.ShopApp.showProductDetail('${shopJsStr(product.id)}')">
                <div class="product-img">${imgHtml}</div>
                <div class="product-info">
                    <div class="product-name">${shopEsc(product.name)}</div>
                    <div class="product-desc">${shopEsc(product.desc)}</div>
                    <div class="product-tags">${tags}</div>
                    <div class="product-price-row">
                        <div class="product-price">¥${shopEsc(product.price)}</div>
                        <button class="add-cart-btn" onclick="event.stopPropagation(); window.ShopApp.addToCart('${shopJsStr(product.id)}')">
    ...
    function editProduct(productId) {
        ...
        if (product.img) {
            ...
            if (preview) {
                preview.innerHTML = `<img src="${shopEsc(product.img)}" style="width:60px;height:60px;object-fit:cover;border-radius:8px;">`;
    ...
    function showProductDetail(productId, mode) {
        ...
        if (product.img) {
            modalImg.innerHTML = `<img src="${shopEsc(product.img)}" style="width:100%;height:100%;object-fit:cover;">`;
        } else {
        ...
        try {
            list.innerHTML = state.cart.map((item, idx) => {
                ...
                const imgHtml = hasImg
                    ? `<img src="${shopEsc(item.img)}" style="width:100%;height:100%;object-fit:cover;">`
                    : (shopEsc(item.icon) || '📦');
                ...
                <div class="cart-item">
                    ...
                    <div class="cart-item-info">
                        <div class="cart-item-name">${shopEsc(item.name) || '未命名商品'}</div>
                        <div class="cart-item-spec">${shopEsc(specText) || ('¥' + (item.price || 0))}</div>
    ...
    function renderOrders() {
        ...
        list.innerHTML = state.orders.map(order => {
            ...
            const cancelBtnHtml = isCancelled ? '' : `<button class="order-btn danger" onclick="window.ShopApp.cancelOrder('${shopJsStr(order.id)}')">取消订单</button>`;
            return `
                <div class="order-item">
                    <div class="order-header">
                        <span style="font-size:0.75rem;color:var(--text-light);">订单号: ${shopEsc(order.id)}</span>
                        <span class="order-status ${escAttr(order.status)}">${shopEsc(statusText)}</span>
                    </div>
                    <div class="order-product">
                        <div class="order-product-img">${shopEsc(order.icon) || '📦'}</div>
                        <div class="order-product-info">
                            <div class="order-product-name">${shopEsc(order.name)}</div>
                            <div class="order-product-spec">${shopEsc(order.specs) || '默认规格'}</div>
                            <div class="order-product-price">¥${shopEsc(order.price)} x ${shopEsc(order.qty)}</div>
                        </div>
                    </div>
                    <div class="order-footer">
                        ...
                        <div class="order-actions">
                            ${cancelBtnHtml}
                            ${order.replies && order.replies.length > 0 ? `<button class="order-btn" onclick="window.ShopApp.showReplyModal('${shopJsStr(order.id)}')">查看回复</button>` : ''}
                            ${!isCancelled ? `<button class="order-btn primary" onclick="window.ShopApp.rebuy('${shopJsStr(order.productId)}')">再次购买</button>` : ''}
                        </div>
                    </div>
                    ${order.remark ? `<div class="order-remark">${shopEsc(order.remark)}</div>` : ''}
                </div>
            `;
        }).join('');
    }
    ...
    function getProductThumbHtml(product, size) {
        ...
        if (product.img && typeof product.img === 'string' && product.img.startsWith('data:')) {
            return `<img src="${shopEsc(product.img)}" style="width:${s}px;height:${s}px;object-fit:cover;border-radius:6px;">`;
        }
        return `<span style="font-size:${s * 0.5}px;">${shopEsc(product.icon) || '📦'}</span>`;
    }
    ...
    function buildShareCard(product, tag, tagColor) {
        ...
        return `<div style="background:linear-gradient(135deg,#fff5f5,#fff);border:1px solid #ffe0e0;border-radius:8px;padding:4px 6px;display:flex;gap:5px;align-items:center;width:260px;">
            ...
            <div style="flex:1;min-width:0;line-height:1.15;">
                <div style="font-size:0.72rem;color:#666;">${shopEsc(product.name)}</div>
    ...
    async function askTAPay() {
        ...
        const payHtml = `<div style="background:linear-gradient(135deg,#fff8f0,#fff);border:1px solid #ffd8b0;border-radius:8px;padding:4px 6px;display:flex;gap:5px;align-items:center;width:260px;">
            ...
            <div style="flex:1;min-width:0;line-height:1.2;">
                ...
                <div style="font-size:0.72rem;font-weight:600;color:#333;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${shopEsc(product.name)}</div>
    ...
    // 拼到内联 onclick 单引号字符串里的参数：先过 HTML 属性级转义，再把 JS 字符串里的单引号转义掉
    function shopJsStr(s) {
        return String(s == null ? '' : s)
            .replace(/&/g, '&amp;')
            .replace(/"/g, '&quot;')
            .replace(/</g, '&lt;')
