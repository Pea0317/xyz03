 ...
 *   id: 时间戳,
 *   ts: 开始时间戳（用于排序、日期计算）,
 *   mode: 'study' | 'work' | 'exercise' | 'sleep' | 'music' | 'eat' | 'moyu',
 *   duration: 秒数,
...
(function() {
    ...
    const MODE_CONFIG = {
        ...
        music:    { name: '听歌', shortName: '听歌', icon: 'fa-headphones', sticker: '🎵' },
        eat:      { name: '吃饭', shortName: '吃饭', icon: 'fa-utensils',   sticker: '🍚' },
        moyu:     { name: '摸鱼', shortName: '摸鱼', icon: 'fa-fish',       sticker: '🎣' }
    };
    ...
    window.addCompanionDiaryEntry = async function(entry) {
        ...
        const rec = {
            ...
            songs: entry.songs || 0,
            userSwitches: entry.userSwitches || 0,
            partnerSwitches: entry.partnerSwitches || 0
        };
        ...
        months.forEach(mo => {
            ...
            if (entries.length === 0) {
                ...
                entries.forEach(e => {
                    ...
                    const musicExtraHtml = (!isMissed && e.mode === 'music' && (e.songs || 0) > 0)
                        ? '<span class="cd-time-dur">共听 ' + e.songs + ' 首</span>'
                        : '';
                    const chatExtraHtml = (!isMissed && (e.mode === 'eat' || e.mode === 'moyu') && (e.chatCount || 0) > 0)
                        ? '<span class="cd-time-dur">对话 ' + e.chatCount + ' 次</span>'
                        : '';
    ...
    function renderStats() {
        ...
        // 种类（仅正常记录）
        const modeCnt = { study: 0, work: 0, exercise: 0, sleep: 0, music: 0, eat: 0, moyu: 0 };
        ...
        const modeData = [
            ...
            { label: '听歌', value: modeCnt.music,    color: modeColors.music },
            { label: '吃饭', value: modeCnt.eat,      color: modeColors.eat },
            { label: '摸鱼', value: modeCnt.moyu,     color: modeColors.moyu }
        ];
