window.switchStatsTab = function(tab) {
    ...
    var redpacketPanel = document.getElementById('redpacket-panel');
    ...
    if (redpacketPanel) redpacketPanel.style.display = 'none';
    ...
    if (tab === 'stats') {
        ...
        if (redpacketPanel) redpacketPanel.style.display = 'block';
        var rpContainer = document.getElementById('redpacket-records');
        if (rpContainer && typeof window.renderRedPacketRecords === 'function') {
            window.renderRedPacketRecords(rpContainer);
        }
    } else {
...
function renderGroupMembersList() {
    ...
    list.innerHTML = groupChatSettings.members.map(function(m, i) {
        var avatarHtml = m.avatar
            ? '<img src="' + escAttr(m.avatar) + '" style="width:36px;height:36px;border-radius:50%;object-fit:cover;">'
        ...
        return '<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--primary-bg);border:1px solid var(--border-color);border-radius:10px;">'
            + avatarHtml
            + '<span style="flex:1;font-size:13px;font-weight:500;">' + String(m.name == null ? '' : m.name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</span>'
...
window._runMsgSearch = function() {
    ...
    resultsEl.innerHTML = filtered.map(function(msg) {
        ...
        var avatarHtml = avatar
            ? '<img src="' + escAttr(avatar) + '" style="width:34px;height:34px;border-radius:50%;object-fit:cover;flex-shrink:0;">'
        ...
        if (msg.image) contentHtml += '<img src="' + escAttr(msg.image) + '" style="max-width:120px;max-height:90px;border-radius:8px;display:block;margin-top:5px;cursor:pointer;" onclick="if(typeof viewImage===\'function\')viewImage(\'' + jsStr(msg.image) + '\')" loading="lazy">';
        ...
        return '<div style="display:flex;align-items:flex-start;gap:10px;padding:10px 12px;border-radius:12px;background:var(--primary-bg);border:1px solid var(--border-color);margin-bottom:8px;cursor:pointer;" onclick="if(typeof scrollToMessage===\'function\')scrollToMessage(' + msg.id + ')">'
            ...
            + '<span style="font-size:12px;font-weight:600;color:var(--accent-color);">' + String(name == null ? '' : name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</span>'
