(function() {
    var ov = document.getElementById('mood-selector-overlay');
...
function saveMoodData() {
    localforage.setItem(getStorageKey('moodCalendar'), moodData).catch(() => {});
    window.moodData = moodData;
    renderMoodCalendar();
    if (typeof window._refreshMoodDisplays === 'function') {
        try { window._refreshMoodDisplays(); } catch (e) { }
    }
}
...
function updateDualMoodStats(stats) {
    ...
    container.innerHTML = `
        ...
        <div class="mood-stat-group">
            ...
            <div style="font-size:11px; color:var(--text-secondary); display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                <span>记录天数: ${myTotal}</span>
                <button class="mood-change-btn" onclick="window.changeTodayMood('me')"><i class="fas fa-pencil-alt"></i> 更改今日心情</button>
            </div>
            ${myBarHTML}
        </div>
        ...
        <div class="mood-stat-group">
            ...
            <div style="font-size:11px; color:var(--text-secondary); display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                ...
                <button class="mood-change-btn" onclick="window.changeTodayMood('partner')"><i class="fas fa-pencil-alt"></i> 更改今日心情</button>
            </div>
            ${partnerBarHTML}
        </div>
    `;
...
window.changeTodayMood = function(who) {
    openMoodSelector(formatDateStr(new Date()), who === 'partner' ? 'partner' : 'me');
};
...
function _showMoodImportPicker(data) {
    ...
    const moodImpCancelBtn = overlay.querySelector('#mood-imp-cancel');
    const moodImpConfirmBtn = overlay.querySelector('#mood-imp-confirm');
