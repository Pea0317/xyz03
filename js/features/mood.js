    ...
    list.innerHTML = anniversaries.map(anniversary => {
        ...
        <div class="anniversary-card ${typeClass}" data-id="${anniversary.id}">
            <div style="display:flex; justify-content:space-between; align-items:center; width:100%;">
                <div class="ann-info">
                    <div class="ann-name">
                        ${String(anniversary.name == null ? '' : anniversary.name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')} 
...
function checkPartnerDailyMood() {
    ...
    // 每个自然日的第一次检查，必然为对方抽一个有效的心情（内置 + 自定义字卡池）
    // 之前：20% 概率"当天不抽"，且 partnerChecked 一旦写成 true 当天就不再补抽；
    // 加上早安弹窗可能早于 moodData 加载完成就构建，会导致连续多天显示「对方还没有记录」。
    // 现在：只要今天还没有对方心情就补抽，且先从池里过滤出合法项，避免抽到畸形/已删的字卡。
    if (!moodData[dateStr].partner) {
        moodData[dateStr].partnerChecked = true;
        const pool = getAllMoodOptions().filter(m => m && m.key);
        if (pool.length > 0) {
            const randomMood = pool[Math.floor(Math.random() * pool.length)];
...
window._setMoodData = function(importedMoodData, importedCustomMoodOptions) {
    if (importedMoodData && typeof importedMoodData === 'object') {
        Object.keys(importedMoodData).forEach(function(dateKey) {
            if (dateKey === '__proto__' || dateKey === 'constructor' || dateKey === 'prototype') return;
            if (!moodData[dateKey]) {
...
function getAllMoodOptions() {
    const customs = Array.isArray(customMoodOptions) ? customMoodOptions : [];
    return [...MOOD_OPTIONS, ...customs];
}
...
function createMoodDot(moodObj, note, isPartner) {
    ...
    if (isPartner) {
        dot.innerHTML = `
            <span class="mood-kaomoji-span">${escapeHtml(moodObj.kaomoji)}</span>
        ...
        dot.innerHTML = `
            <span class="mood-kaomoji-span">${escapeHtml(moodObj.kaomoji)}</span>
            <span class="mood-text-span" style="margin-left:2px;">${escapeHtml(displayText)}</span>
        `;
    }
    return dot;
}
function updateDualMoodStats(stats) {
        ...
        const segments = Object.keys(moodCounts)
            .map(key => {
                ...
                if (moodObj) {
                    ...
                    return `<div class="mood-bar-segment" style="width: ${percentage}%; background-color: ${moodObj.color};" title="${_escapeHtml(moodObj.label)}: ${count}天"></div>`;
                }
                return ''; 
            })
            .join(''); 
    ...
    container.innerHTML = `
        ...
        <div class="mood-stat-group">
            <div class="mood-stat-title">
                ...
                <div class="dominant-mood-tag">
                    <span style="color:${myDominant.color}; font-weight:bold;">${_escapeHtml(myDominant.kaomoji)} ${_escapeHtml(myDominant.label)}</span>
                </div>
            </div>
        ...
        <div class="mood-stat-group">
            <div class="mood-stat-title">
                <span>${_escapeHtml(pName)}的心情</span>
                <div class="dominant-mood-tag">
                    <span style="color:${partnerDominant.color}; font-weight:bold;">${_escapeHtml(partnerDominant.kaomoji)} ${_escapeHtml(partnerDominant.label)}</span>
                </div>
            </div>
            <div style="font-size:11px; color:var(--text-secondary); display:flex; justify-content:space-between;">
...
window.editStatsWeather = function(el, who) {
    ...
    function save() {
        ...
        saveMoodData();
        el.innerHTML = val ? `<span>${_escapeHtml(val)}</span>` : `<span style="opacity:0.4;">+ 天气</span>`;
    }
    input.addEventListener('blur', save);
    input.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') { e.preventDefault(); save(); }
        if (e.key === 'Escape') { el.innerHTML = current ? `<span>${_escapeHtml(current)}</span>` : `<span style="opacity:0.4;">+ 天气</span>`; }
    });
};
        ...
        try {
            if (selCal && hasCalendar) {
                Object.keys(data.moodCalendar).forEach(dateStr => {
                    if (dateStr === '__proto__' || dateStr === 'constructor' || dateStr === 'prototype') return;
                    if (!moodData[dateStr]) moodData[dateStr] = {};
                    if (data.moodCalendar[dateStr] && typeof data.moodCalendar[dateStr] === 'object') {
                        Object.keys(data.moodCalendar[dateStr]).forEach(mk => {
                            if (mk === '__proto__' || mk === 'constructor' || mk === 'prototype') return;
                            moodData[dateStr][mk] = data.moodCalendar[dateStr][mk];
                        });
                    }
                });
            }
...
function renderMoodOptionsGrid(targetKey) {
    ...
    optionsGrid.innerHTML = allMoods.map(mood => {
        ...
        <div class="mood-option-btn${isCustom ? ' mood-option-custom' : ''}" 
            ...
            <div class="mood-kaomoji" style="${isSelected ? 'color:#fff' : `color:${mood.color}`}">${_escapeHtml(mood.kaomoji)}</div>
            <div class="mood-label">${_escapeHtml(mood.label)}</div>
