...
(function () {
    ...
    const MODES = {
        ...
        music:    {
            ...
            times: [5,10,15,20,25,30,'rest'],
            restLabel: '随心听',
            inviteTimes: [10, 15, 20, 25],
        },
        eat:      {
            label: '一起吃饭',
            icon:  'fa-utensils',
            hint:  '正在一起吃饭 · 好好享用',
            times: [],                          // 一起吃饭无需选时间，直接邀请
            restLabel: '开饭啦',
            inviteTimes: [],                    // 梦角邀请也不带时间
            noTime: true,
        },
        moyu:     {
            label: '一起摸鱼',
            icon:  'fa-fish',
            hint:  '正在一起摸鱼 · 偷个懒吧',
            times: [10, 20, 30, 60, 'rest'],    // 时间参考「好好休息」的正计时设计
            restLabel: '摸个痛快',
            inviteTimes: [20, 30, 'rest'],
        },
    };
    ...
    let companionData = {
        backgrounds: { study: [], work: [], exercise: [], sleep: [], music: [], eat: [], moyu: [] },
        voices:      { study: [], work: [], exercise: [], sleep: [], music: [], eat: [], moyu: [] },
        noises:      { study: [], work: [], exercise: [], sleep: [], music: [], eat: [], moyu: [] },
        selectedBg:  { study: null, work: null, exercise: null, sleep: null, music: null, eat: null, moyu: null }, // 选中的背景 id
        lastNoiseChoice: { study: null, work: null, exercise: null, sleep: null, music: null, eat: null, moyu: null },
        lastPlayMode:    { study: 'single', work: 'single', exercise: 'single', sleep: 'single', music: 'single', eat: 'single', moyu: 'single' },
        history: []
    };
    ...
    function _emptyData() {
        return {
            ...
            voices:      { study: [], work: [], exercise: [], sleep: [], music: [], eat: [], moyu: [] },
            noises:      { study: [], work: [], exercise: [], sleep: [], music: [], eat: [], moyu: [] },
            selectedBg:  { study: null, work: null, exercise: null, sleep: null, music: null, eat: null, moyu: null },
            lastNoiseChoice: { study: null, work: null, exercise: null, sleep: null, music: null, eat: null, moyu: null },
            lastPlayMode:    { study: 'single', work: 'single', exercise: 'single', sleep: 'single', music: 'single', eat: 'single', moyu: 'single' },
            history: []
        };
    ...
    function openCompanionModal() {
        ...
        modal.innerHTML = `
            ...
            ">
                ...
                <div id="companion-cards-wrap" style="display:grid;grid-template-columns:repeat(2,1fr);gap:14px;padding:6px 4px;">
                    ...
                    `).join('')}
                        <div id="companion-card-wuziqi-dyn" class="wzq-dyn-card" style="
                            background:var(--primary-bg, #fafafa);border-radius:14px;padding:22px 12px;cursor:pointer;
                            border:1px solid var(--border-color, rgba(0,0,0,0.06));
                            display:flex;flex-direction:column;align-items:center;gap:10px;
                            transition:all 0.25s ease;user-select:none;
                        ">
                            <div style="
                                ...
                                display:flex;align-items:center;justify-content:center;
                            ">
                                <i class="fas fa-chess-board" style="font-size:24px;color:var(--accent-color, #c5a47e);"></i>
                            </div>
                            <span style="font-size:14px;font-weight:600;color:var(--text-primary, #1a1a1a);">五子棋</span>
                        </div>
                </div>
        ...
        // 五子棋模式卡（独立于 data-mode，路由到 wuziqi 模块）
        const wzqCard = modal.querySelector('#companion-card-wuziqi-dyn');
        if (wzqCard) {
            wzqCard.addEventListener('click', e => {
                e.stopPropagation();
                if (window.__wuziqiModule && window.__wuziqiModule.start) window.__wuziqiModule.start();
            });
            wzqCard.addEventListener('mouseenter', () => {
                wzqCard.style.transform = 'translateY(-3px)';
                wzqCard.style.borderColor = 'var(--accent-color, #c5a47e)';
                wzqCard.style.boxShadow = '0 10px 24px rgba(0,0,0,0.08)';
            });
            wzqCard.addEventListener('mouseleave', () => {
                wzqCard.style.transform = '';
                wzqCard.style.borderColor = 'var(--border-color, rgba(0,0,0,0.06))';
                wzqCard.style.boxShadow = '';
            });
        }
    ...
    const INVITE_LINES = {
        ...
        music: ['一起听歌吗？', '想分享一首歌给你，陪我一起听？', '要不要一起听会儿歌？'],
        eat: ['一起吃饭吗？', '我饿了，陪我吃饭好吗？', '到饭点了，一起吃饭？'],
        moyu: ['一起摸会儿鱼？', '忙里偷个闲，陪我摸鱼？', '工作累了，一起摸鱼放松下？'],
    };
    ...
    async function selectMode(mode) {
        ...
        // 无时间场景（如一起吃饭）：跳过时间选择，直接进入邀请等待（正计时）
        if (cfg && cfg.noTime) {
            isCountdown = false;
            timerSeconds = 0;
            totalSeconds = 0;
    ...
    function enterWithInviteTime(mode, time) {
        ...
        // time 为 null/undefined 表示无时间场景（一起吃饭）→ 正计时
        if (time === 'rest' || time == null) {
            isCountdown = false;
            timerSeconds = 0;
            totalSeconds = 0;
        } else {
    ...
    function showCompanionInviting(mode) {
        ...
        let userTimeText;
        if (cfg.noTime) {
            userTimeText = ''; // 无时间场景：不显示时间
        } else if (!isCountdown) {
            userTimeText = cfg.restLabel || '好好休息';
        } else {
        ...
        overlay.innerHTML = `
            ...
            ">
                ...
                <div style="font-size:13px;color:rgba(255,255,255,0.6);display:flex;align-items:center;gap:8px;">
                    ...
                    <span>邀请${cfg.label}${userTimeText ? ' · ' + userTimeText : ''}</span>
    ...
    async function showIncomingCompanion(mode) {
        ...
        // 梦角自选时间（从 inviteTimes 池里随机选一个）；无时间场景则不带时间
        const inviteTime = cfg.noTime ? null : pickRandom(cfg.inviteTimes || [25]);

        // 拼接邀请文案：智能处理"陪你"是否已经在台词里 + rest/无时间特殊处理
        let line;
        if (cfg.noTime || inviteTime == null) {
            // 一起吃饭等无时间场景：直接用台词，不追加时间
            line = baseLine;
        } else if (inviteTime === 'rest') {
            // 好好休息 / 摸个痛快 等正计时模式：按场景给一句更自然的话
            line = (mode === 'sleep') ? '陪你一起睡到自然醒吧' : ('陪你' + getSceneName() + '到自然结束吧');
        } else {
        ...
        overlay.innerHTML = `
            <div style="display:flex;flex-direction:column;align-items:center;gap:20px;color:#fff;max-width:300px;padding:0 20px;animation:companionPopIn 2s ease;">
                <div style="
                    ...
                    border:2px solid rgba(255,255,255,0.15);
                ">${avatarHtml}</div>
                <div style="font-size:18px;font-weight:600;letter-spacing:1px;">${escapeHtml(partnerName)}</div>
                <div style="
    ...
    function renderSetupVoiceListDyn(modal) {
        ...
        list.innerHTML = voices.map((v, i) => `
            <div style="display:flex;align-items:center;gap:8px;background:rgba(var(--accent-color-rgb,197,164,126),0.07);border-radius:10px;padding:8px 10px;">
                ...
                <input type="text" value="${escapeHtml(v.name || '')}" data-idx="${i}" class="setup-dyn-voice-name"
        ...
        const timesHtml = cfg.times.map(t => {
            if (t === 'rest') {
                ...
                ">
                    ...
                    <span style="font-size:13px;font-weight:600;color:var(--text-primary, #1a1a1a);">${cfg.restLabel || '好好休息'}</span>
                </button>`;
            }
    ...
    function openCompanionPage(opts) {
        ...
        if (timerLabel) {
            const labels = { study: 'STUDY', work: 'WORK', exercise: 'EXERCISE', sleep: 'SLEEP', music: 'MUSIC', eat: 'EAT', moyu: 'MOYU' };
        ...
        if (!bg) {
            ...
            if (currentMode === 'music') {
                ...
                fallback.style.cssText = `
                    ...
                    background:
                        radial-gradient(circle at 18% 22%, rgba(186,146,255,0.28) 0%, transparent 55%),
                        radial-gradient(circle at 82% 78%, rgba(255,124,163,0.28) 0%, transparent 55%),
                        linear-gradient(160deg, #241B37 0%, #3A2A52 45%, #241B37 100%);
                `;
            } else {
                fallback.style.cssText = `
                    position:absolute;inset:0;
            ...
            if (page && currentMode !== 'music') page.classList.add('companion-light-bg');
            return;
        }
    ...
    function closeCompanionPage(opts) {
        ...
        // 一起听歌的结束统计（次数要在这里先取，之后 stopCompanionMusic 会清零）
        const _diaryMusic = (_diaryMode === 'music') ? getCompanionMusicStats() : null;
        ...
        try {
            if (_diaryMode && _diaryDurSec >= 30 && typeof window.addCompanionDiaryEntry === 'function') {
                ...
                window.addCompanionDiaryEntry({
                    ...
                    partnerNote: partnerNote,
                    userNote: '',
                    chatCount: _sessionDialogue.length,
                    songs: _diaryMusic ? _diaryMusic.songs : undefined,
                    userSwitches: _diaryMusic ? _diaryMusic.userSwitch : undefined,
                    partnerSwitches: _diaryMusic ? _diaryMusic.partnerSwitch : undefined
                });
            }
        } catch (e) {
        ...
        // 一起听歌：结束时显示一次小结
        if (_diaryMusic && _diaryDurSec >= 1 && typeof showNotification === 'function') {
            showNotification(
                `本次一起听歌：共听 ${_diaryMusic.songs} 首 · 你切歌 ${_diaryMusic.userSwitch} 次 · ${getPartnerName()} 切歌 ${_diaryMusic.partnerSwitch} 次`,
                'info',
                5000
            );
        }

        // 一起吃饭 / 一起摸鱼：结束时显示一次小结（对话次数 + 发起人）
        if ((_diaryMode === 'eat' || _diaryMode === 'moyu') && _diaryDurSec >= 1 && typeof showNotification === 'function') {
            const initiatorName = _diaryInitiator === 'partner' ? getPartnerName() : getMyName();
            showNotification(
                `本次${getSceneName()}：一起对话 ${_sessionDialogue.length} 次 · 由 ${initiatorName} 发起邀请`,
                'info',
                5000
            );
        }
    ...
    // ─── 一起听歌：伴随播放器 ──────────────────────────────────────────────────
    // 悬浮播放器不可用时的兜底歌单（正常情况会直接复用 window._playerSongList）
    const MUSIC_FALLBACK_SONGS = [
        { title: '偏爱', sub: '等你的依赖 对你的偏爱', url: 'https://music.163.com/song/media/outer/url?id=5238992.mp3' },
        ...
        { title: '永远永远', sub: '你爱过我就已足够', url: 'https://files.catbox.moe/2lcm70.mp3' },
    ...
    let _musicPartnerTimer = null;
    let _musicStats = { seen: new Set(), userSwitch: 0, partnerSwitch: 0 };

    // 梦角切歌 / 调音量时的台词
    const PARTNER_MUSIC_LINES = {
        next: ['这首也好好听～', '切到一首我很喜欢的！', '听这首，我超爱'],
        prev: ['刚刚那首还想再听一遍……', '退回上一首吧～'],
        volumeUp: ['把音量调大了一点点', '音乐声大一点更有感觉'],
        volumeDown: ['音量小一点，怕吵到你', '调小了音量，这样刚好'],
        seekForward: ['快进了一小段，想听听后面', '往后拉了一点点，继续听～'],
        seekBackward: ['倒回去一点点，前面那段还想听', '往回调了下进度，把刚才那段再听一遍'],
    };

    async function _loadCompanionSongs() {
        // 优先复用悬浮播放器已经加载好的歌单（含用户自定义歌曲）
        if (window._playerSongList && Array.isArray(window._playerSongList) && window._playerSongList.length) {
            return window._playerSongList;
        }
        try {
            const key = (window.APP_PREFIX || '') + 'customSongs';
            const saved = await localforage.getItem(key);
            if (Array.isArray(saved) && saved.length) return saved;
            if (typeof safeGetItem === 'function') {
                const legacyStr = safeGetItem(key);
                if (legacyStr) {
                    const arr = JSON.parse(legacyStr);
                    if (Array.isArray(arr) && arr.length) return arr;
                }
            }
        } catch (e) { console.warn('[companion] 加载歌单失败', e); }
        return MUSIC_FALLBACK_SONGS;
    }

    function _musicFormatTime(sec) {
    ...
    function _musicRefreshPlayingUI() {
        const area = document.getElementById('companion-music-area');
        if (!area) return;
        const playIcon = area.querySelector('#cmp-play i');
        if (playIcon) playIcon.className = _musicPlaying ? 'fas fa-pause' : 'fas fa-play';
        area.classList.toggle('playing', _musicPlaying);
        const disc = document.getElementById('cmp-disc');
        if (disc) disc.classList.toggle('spin', _musicPlaying);
    }

    function _musicPlayAt(idx, by) {
        if (!_musicSongList.length) return;
        if (idx >= _musicSongList.length) idx = 0;
        if (idx < 0) idx = _musicSongList.length - 1;
        _musicIndex = idx;

        const song = _musicSongList[idx];
        const titleEl = document.getElementById('cmp-title');
        const subEl = document.getElementById('cmp-sub');
        if (titleEl) titleEl.textContent = song.title || '未命名';
        if (subEl) subEl.textContent = song.sub || '未知艺术家';

        if (!_musicAudio) return;
        _musicStats.seen.add(idx);
        ...
        if (isCloud) {
            window.CloudMedia.fetchUrl(song.url).then((blobUrl) => {
                _musicAudio.src = blobUrl;
                if (_musicPlaying) _musicAudio.play().catch(() => {});
            }).catch((e) => {
                if (typeof showNotification === 'function') showNotification('云端音频加载失败', 'error');
            });
        } else if (song.url) {
            _musicAudio.src = song.url;
            if (_musicPlaying) _musicAudio.play().catch(() => {});
        }
    }

    function _musicNext(by) {
        if (!_musicSongList.length) return;
        _musicIndex = (_musicIndex + 1) % _musicSongList.length;
        if (by === 'partner' || by === 'user') _musicStats[by === 'user' ? 'userSwitch' : 'partnerSwitch']++;
        _musicPlayAt(_musicIndex, by);
    }

    function _musicPrev(by) {
        if (!_musicSongList.length) return;
        _musicIndex = (_musicIndex - 1 + _musicSongList.length) % _musicSongList.length;
        if (by === 'partner' || by === 'user') _musicStats[by === 'user' ? 'userSwitch' : 'partnerSwitch']++;
        _musicPlayAt(_musicIndex, by);
    }
    ...
    function _musicTogglePlay() {
        ...
        if (_musicPlaying) {
            ...
            if (p !== undefined) {
                p.then(() => { _musicPlaying = true; _musicRefreshPlayingUI(); })
                 .catch(() => { if (typeof showNotification === 'function') showNotification('播放失败，请检查歌曲链接', 'error'); });
            } else {
                _musicPlaying = true;
            }
        }
        _musicRefreshPlayingUI();
    }

    function _musicChangeVolume(delta, by) {
        _musicVolume = Math.min(1, Math.max(0.02, _musicVolume + delta));
        if (_musicAudio) _musicAudio.volume = _musicVolume;
        if (by === 'partner') {
            const line = delta > 0 ? pickRandom(PARTNER_MUSIC_LINES.volumeUp) : pickRandom(PARTNER_MUSIC_LINES.volumeDown);
            showCompanionBubble({ sender: 'partner', text: line, image: null });
        }
    }

    function _musicPartnerSwitch(isNext) {
        // 对端 30% 概率行动：上一首 / 下一首随机
        if (Math.random() < 0.7) {
            _musicNext('partner');
        } else {
            _musicPrev('partner');
        }
        const song = _musicSongList[_musicIndex];
        const line = isNext ? pickRandom(PARTNER_MUSIC_LINES.next) : pickRandom(PARTNER_MUSIC_LINES.prev);
        const title = song ? song.title : '';
        showCompanionBubble({ sender: 'partner', text: title ? `${line}《${title}》` : line, image: null });
    }

    function _musicPartnerSeek() {
        // 对端（调度中有 5% 概率触发）调节当前歌曲进度：随机往前或往后跳一段
        if (!_musicAudio || !_musicAudio.duration || _musicAudio.duration <= 0) return;
        const dur = _musicAudio.duration;
        const forward = Math.random() < 0.5;
        const jump = dur * (0.12 + Math.random() * 0.28); // 12%~40% 的时长
        const target = Math.max(0, Math.min(dur - 1, _musicAudio.currentTime + (forward ? jump : -jump)));
        try { _musicAudio.currentTime = target; } catch (e) {}
        const song = _musicSongList[_musicIndex];
        const line = forward ? pickRandom(PARTNER_MUSIC_LINES.seekForward) : pickRandom(PARTNER_MUSIC_LINES.seekBackward);
        const title = song ? song.title : '';
        showCompanionBubble({ sender: 'partner', text: title ? `${line}《${title}》` : line, image: null });
    }

    function _scheduleMusicPartnerAction() {
        clearTimeout(_musicPartnerTimer);
        _musicPartnerTimer = setTimeout(() => {
            const page = document.getElementById('companion-page');
            if (!page || !page.classList.contains('active') || currentMode !== 'music') return;
            const roll = Math.random();
            if (roll < 0.30) {
                // 30% 切上一首 / 下一首
                _musicPartnerSwitch(true);
            } else if (roll < 0.45) {
                // 15% 调整音量（双方共享同一音量，实时生效）
                _musicChangeVolume(Math.random() < 0.5 ? 0.15 : -0.15, 'partner');
            } else if (roll < 0.50) {
                // 5% 调节当前歌曲进度（随机往前 / 往后）
                _musicPartnerSeek();
            }
            // 否则啥都不做继续听
            _scheduleMusicPartnerAction();
        }, 18000 + Math.random() * 16000);
    }

    function getCompanionMusicStats() {
        return {
            songs: _musicStats.seen.size,
            userSwitch: _musicStats.userSwitch,
            partnerSwitch: _musicStats.partnerSwitch
        };
    }

    function _bindMusicUI(area) {
        const audio = area.querySelector('#companion-music-audio');
        _musicAudio = audio;
        _musicAudio.volume = _musicVolume;

        const playBtn = area.querySelector('#cmp-play');
        const nextBtn = area.querySelector('#cmp-next');
        const prevBtn = area.querySelector('#cmp-prev');
        const volUpBtn = area.querySelector('#cmp-vol-up');
        const volDownBtn = area.querySelector('#cmp-vol-down');
        const bar = area.querySelector('#cmp-bar');
        const fill = area.querySelector('#cmp-bar-fill');
        const handleEl = area.querySelector('#cmp-bar-handle');
        const curEl = area.querySelector('#cmp-current');
        const durEl = area.querySelector('#cmp-duration');

        // 拖动进度条：双方都能拖，实时预览，松手时定位
        let barDragging = false;
        const setBarUI = (ratio) => {
            if (!isFinite(ratio)) return;
            ratio = Math.max(0, Math.min(1, ratio));
            if (fill) fill.style.width = (ratio * 100) + '%';
            if (handleEl) handleEl.style.left = (ratio * 100) + '%';
            if (curEl) curEl.textContent = _musicFormatTime(ratio * (_musicAudio ? _musicAudio.duration || 0 : 0));
        };
        ...
        const commitSeek = (e) => {
            ...
            barDragging = false;
            bar.classList.remove('cmp-bar-dragging');
            if (_musicAudio && _musicAudio.duration) {
                try { _musicAudio.currentTime = ratioFromEvent(e) * _musicAudio.duration; } catch (err) {}
            }
        };
        if (bar) {
            bar.addEventListener('pointerdown', (e) => {
                ...
                e.preventDefault();
                barDragging = true;
                bar.classList.add('cmp-bar-dragging');
                try { bar.setPointerCapture(e.pointerId); } catch (err) {}
                setBarUI(ratioFromEvent(e));
            });
            ...
            bar.addEventListener('pointercancel', commitSeek);
            // 无 PointerEvent 的旧环境兜底：单击跳转
        ...
        if (nextBtn) nextBtn.addEventListener('click', (e) => { e.stopPropagation(); _musicNext('user'); });
        if (prevBtn) prevBtn.addEventListener('click', (e) => { e.stopPropagation(); _musicPrev('user'); });
        if (volUpBtn) volUpBtn.addEventListener('click', (e) => { e.stopPropagation(); _musicChangeVolume(0.15, 'user'); });
        if (volDownBtn) volDownBtn.addEventListener('click', (e) => { e.stopPropagation(); _musicChangeVolume(-0.15, 'user'); });

        _musicAudio.addEventListener('timeupdate', () => {
            if (barDragging) return; // 拖动时交给预览，松手统一跳转
            if (!_musicAudio.duration) return;
            setBarUI(_musicAudio.currentTime / _musicAudio.duration);
            if (durEl) durEl.textContent = _musicFormatTime(_musicAudio.duration);
        });
        _musicAudio.addEventListener('loadedmetadata', () => {
            setBarUI(0);
            if (durEl) durEl.textContent = _musicFormatTime(_musicAudio.duration || 0);
        });
        _musicAudio.addEventListener('ended', () => _musicNext());
        _musicAudio.addEventListener('play', () => { _musicPlaying = true; _musicRefreshPlayingUI(); });
        _musicAudio.addEventListener('pause', () => { _musicPlaying = false; _musicRefreshPlayingUI(); });
    }

    async function startCompanionMusic() {
        stopCompanionMusic();
        _musicStats = { seen: new Set(), userSwitch: 0, partnerSwitch: 0 };

        // 暂停主悬浮播放器，避免两个音乐同时响（同时复位它的播放图标）
        try {
            const mainAudio = document.getElementById('audio');
            const playerEl = document.getElementById('player');
            if (mainAudio && playerEl && playerEl.classList.contains('playing')) {
                mainAudio.pause();
        ...
        area.innerHTML = `
            <div class="companion-music-player">
                <audio id="companion-music-audio" preload="metadata"></audio>
                <div class="cmp-head">
                    <span class="cmp-head-label"><i class="fas fa-headphones"></i> 梦角在听 · 一起听歌中</span>
                    <span class="cmp-equalizer"><span></span><span></span><span></span><span></span></span>
                </div>
                <div class="cmp-row">
                    <div class="cmp-disc" id="cmp-disc"><i class="fas fa-music"></i></div>
                    <div class="cmp-info">
                        <div class="cmp-title" id="cmp-title">正在加载歌单…</div>
                        <div class="cmp-sub" id="cmp-sub"></div>
<div class="cmp-progress">
                                <span class="cmp-time" id="cmp-current">00:00</span>
                                <div class="cmp-bar" id="cmp-bar"><div class="cmp-bar-fill" id="cmp-bar-fill"></div><div class="cmp-bar-handle" id="cmp-bar-handle"></div></div>
                                <span class="cmp-time" id="cmp-duration">00:00</span>
                            </div>
                    </div>
                </div>
                <div class="cmp-controls">
                    <button class="cmp-ctrl" id="cmp-vol-down" title="音量减"><i class="fas fa-volume-down"></i></button>
                    <button class="cmp-ctrl" id="cmp-prev" title="上一首"><i class="fas fa-backward-step"></i></button>
                    <button class="cmp-ctrl cmp-ctrl-play" id="cmp-play" title="播放 / 暂停"><i class="fas fa-play"></i></button>
                    <button class="cmp-ctrl" id="cmp-next" title="下一首"><i class="fas fa-forward-step"></i></button>
                    <button class="cmp-ctrl" id="cmp-vol-up" title="音量加"><i class="fas fa-volume-up"></i></button>
                </div>
            </div>
        `;
        _bindMusicUI(area);

        const songs = await _loadCompanionSongs();
        _musicSongList = songs;
        if (!songs.length) {
            document.getElementById('cmp-title').textContent = '歌单是空的';
            return;
        }
        _musicPlayAt(0);
        // 尝试自动播放（用户刚点过按钮，一般能成功；被拦截则让用户点播放）
        if (_musicAudio) {
            const p = _musicAudio.play();
            if (p !== undefined) p.then(() => { _musicPlaying = true; _musicRefreshPlayingUI(); }).catch(() => {
                _musicRefreshPlayingUI();
            });
        }
        _scheduleMusicPartnerAction();
    ...
    function stopCompanionMusic() {
        clearTimeout(_musicPartnerTimer);
        _musicPartnerTimer = null;
        if (_musicAudio) {
            try { _musicAudio.pause(); _musicAudio.removeAttribute('src'); _musicAudio.load(); } catch (e) {}
            _musicAudio = null;
        }
        _musicPlaying = false;
        const area = document.getElementById('companion-music-area');
    ...
    function handlePageClick(e) {
        ...
        if (e.target.closest('button, input, #companion-timer-area, #companion-exit-confirm, .companion-bubble-voice, #companion-music-area')) return;
        // 涟漪特效（始终响应）
