...
function showEnvelopeReplyPopup(letter) {
    ...
    popup.innerHTML = `
        ...
        <div style="display:flex;align-items:center;gap:10px;">
            ...
            <div>
                <div style="font-size:14px;font-weight:700;color:var(--text-primary);">${escapeHtml(letter.fromPartner ? ((typeof settings !== 'undefined' && settings.partnerName) || '对方') + '给你写了一封信' : '收到了一封回信')}</div>
        ...
        <div style="display:flex;gap:8px;">
            ...
            <button onclick="openEnvelopeAndViewReply('${jsStr(letter.id)}');" style="flex:2;padding:8px 0;border-radius:12px;border:none;background:var(--accent-color);color:#fff;font-size:13px;font-weight:600;cursor:pointer;">立即阅读 ✉</button>
        </div>`;
