...
function _showN({type,postId}){
    ...
    const name=_mPName(),nameEsc=String(name==null?'':name).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'),C={newPost:{icon:'📸',title:`${nameEsc}发了新动态`,sub:'快去看看 Ta 的动态~'},liked:{icon:'❤️',title:`${nameEsc}为你的动态点了赞`,sub:''},commented:{icon:'💬',title:`${nameEsc}评论了你的动态`,sub:'去看看 Ta 说了什么~'},replied:{icon:'💬',title:`${nameEsc}回复了你`,sub:'去看看 Ta 说了什么~'}};
...
function _imgEl(src){
    ...
    const srcEsc=String(src).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/`/g,'&#96;');
    const srcJs=String(src).replace(/\\/g,'\\\\').replace(/'/g,'\\u0027');
    const ca=!src.startsWith('data:')?`onclick="viewImage('${srcJs}')" style="cursor:pointer;"`:'';
    return isCloud?`<img data-lazy-cloud-ref="${srcEsc}" style="width:100%;height:100%;object-fit:cover;" ${ca}>`:`<img src="${srcEsc}" style="width:100%;height:100%;object-fit:cover;" ${ca}>`;
}
function _imgElThumb(src, style){
    ...
    const srcEsc=String(src).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/`/g,'&#96;');
    const srcJs=String(src).replace(/\\/g,'\\\\').replace(/'/g,'\\u0027');
    const ca=`onclick="viewImage('${srcJs}')"`;
    return isCloud?`<img data-lazy-cloud-ref="${srcEsc}" style="${s}" ${ca}>`:`<img src="${srcEsc}" style="${s}" ${ca}>`;
}
...
function _videoThumb(videoSrc, coverSrc){
    ...
    const isCloud=String(cover).indexOf('oss://')===0;
    const coverEsc=String(cover).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/`/g,'&#96;');
    const videoEsc=String(videoSrc).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    const videoJs=String(videoSrc).replace(/\\/g,'\\\\').replace(/'/g,'\\u0027');
    const coverTag=cover?(isCloud?`<img data-lazy-cloud-ref="${coverEsc}" class="cs-video-cover">`:`<img src="${coverEsc}" class="cs-video-cover">`):`<div class="cs-video-cover cs-video-cover-empty"><i class="fas fa-film"></i></div>`;
    return`<div class="cs-video-thumb" onclick="window.openCsVideoPlayer('${videoJs}')">${coverTag}<div class="cs-video-play-btn"><i class="fas fa-play"></i></div></div>`;
}
...
function _mStickerGroupRowHTML(list){
    ...
    list.forEach(g=>{
        ...
        const isCloud=typeof cover==='string' && String(cover).indexOf('oss://')===0;
        const coverEsc=cover?String(cover).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/`/g,'&#96;'):'';
        const gNameEsc=String(g.name==null?'':g.name).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/`/g,'&#96;');
        const inner=cover
            ? (isCloud
                ? `<img loading="lazy" data-cover-ref="${coverEsc}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`
                : `<img loading="lazy" src="${coverEsc}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`)
            : '<i class="fas fa-images" style="font-size:13px;"></i>';
        const isActive=g.id===_mActiveStickerGroup;
        html+=`<button class="my-sticker-group-chip${isActive?' active':''}" data-group-id="${g.id===null?'':escAttr(g.id)}" title="${gNameEsc}">${inner}</button>`;
    });
    html+='</div>';
    return html;
}
...
window._mSelectSticker=function(postId,src){
    ...
    if(pv){
        const isCloud=src.indexOf('oss://')===0;
        const srcEsc=String(src==null?'':src).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
        const pidEsc=String(postId).replace(/\\/g,'\\\\').replace(/'/g,'\\u0027');
        const imgTag=isCloud?`<img data-lazy-cloud-ref="${srcEsc}" style="height:52px;border-radius:8px;">`:`<img src="${srcEsc}" style="height:52px;border-radius:8px;">`;
        pv.innerHTML=`<div style="position:relative;display:inline-block;margin:4px 0;">${imgTag}<button onclick="window._mClearCmtImg('${pidEsc}')" style="position:absolute;top:-4px;right:-4px;width:16px;height:16px;border-radius:50%;background:rgba(0,0,0,.6);border:none;color:#fff;font-size:9px;cursor:pointer;display:flex;align-items:center;justify-content:center;">✕</button></div>`;
...
window._mCommentImgSelected=function(postId,input){
    ...
    optimizeImage(file,600,0.75).then(b64=>{
        ...
        const pv=document.getElementById('cs-cmt-img-prev-'+postId);
        if(pv){const srcEsc=String(b64==null?'':b64).replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;').replace(/>/g,'&gt;');const pidEsc=String(postId).replace(/\\/g,'\\\\').replace(/'/g,'\\u0027');pv.innerHTML=`<div style="position:relative;display:inline-block;margin:4px 0;"><img src="${srcEsc}" style="height:52px;border-radius:8px;"><button onclick="window._mClearCmtImg('${pidEsc}')" style="position:absolute;top:-4px;right:-4px;width:16px;height:16px;border-radius:50%;background:rgba(0,0,0,.6);border:none;color:#fff;font-size:9px;cursor:pointer;display:flex;align-items:center;justify-content:center;">✕</button></div>`;}input.value='';
};
...
function _renderCmtSectionHtml(post){
    ...
    toShow.forEach(c=>{
        ...
        const cName=isP?_mPName():_mMName();const cNameEsc=String(cName==null?'':cName).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');const cNameJs=String(cName==null?'':cName).replace(/\\/g,'\\\\').replace(/'/g,'\\u0027');
        const replyPart=c.replyTo?` <span class="cs-cmt-reply-word">回复</span> <span class="cs-cmt-reply-target">${String(c.replyTo.authorName==null?'':c.replyTo.authorName).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</span>`:'';
        ...
        cmtListHtml+=`<div class="cs-cmt-item" onclick="event.stopPropagation();window._mClickComment('${jsStr(post.id)}','${jsStr(c.id)}','${cNameJs}')"><div class="cs-cmt-content"><span class="cs-cmt-name">${cNameEsc}</span>${replyPart}：${c.text?`<span class="cs-cmt-text">${String(c.text).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}${dot}</span>`:''}${imgHtml}</div></div>`;
    });
...
function _renderCard(post){
    ...
    return`<div class="cs-post" data-pid="${escAttr(post.id)}">
        <div class="cs-post-top">
            <div class="cs-post-av">${_avEl(isPartner,36)}</div>
            <div class="cs-post-name">${String(name==null?'':name).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}${hasNew?'<span style="width:7px;height:7px;background:var(--accent-color);border-radius:50%;display:inline-block;margin-left:6px;vertical-align:middle;"></span>':''}</div>
            ${canDel?`<button class="cs-post-del" onclick="event.stopPropagation();window._mDeletePost('${jsStr(post.id)}')"><i class="fas fa-trash-alt"></i></button>`:'<div style="width:24px;"></div>'}
        </div>
        <div class="cs-post-body">${String(post.text==null?'':post.text).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</div>
...
function _updateDaysCounter() {
    ...
    function render(labelStr, daysStr) {
        textEl.innerHTML =
            '<span class="cs-days-label" id="cs-days-label">' + escapeHtml(labelStr) + '</span>'
            + '<div class="cs-days-count">'
            + '<span class="cs-days-num" id="cs-days-num">' + escapeHtml(daysStr) + '</span>'
