...
(function() {
    ...
    const STORAGE_KEY = (window.APP_PREFIX || '') + 'ta_phone_collections';
    ...
    // 扫描历史内容（只扫描未标记的消息；按 wz2 按会话存储读取）
    function scanHistory() {
        var sessionArr = (typeof sessionList !== 'undefined') ? sessionList : (window.sessionList || []);
        if (!Array.isArray(sessionArr)) sessionArr = [];
        var tasks = sessionArr.map(function(session) {
            var key = (window.APP_PREFIX || '') + session.id + '_chatMessages';
            var p;
            if (window.localforage && typeof window.localforage.getItem === 'function') {
                p = window.localforage.getItem(key);
            } else {
            ...
            return p.then(function(raw) {
                ...
                if (typeof msgs === 'string') {
                    try { msgs = JSON.parse(msgs); } catch (e) { msgs = null; }
                }
                if (!Array.isArray(msgs)) return;
                msgs.forEach(function(msg) {
                    if (!msg || msg.taPhoneCollected) return;
                    if (msg.sender === 'user' && msg.text && typeof msg.text === 'string' && msg.text.trim()) {
                        var content = msg.text.trim();
                        var ts = typeof msg.timestamp === 'number' ? msg.timestamp : (msg.timestamp && msg.timestamp.getTime ? msg.timestamp.getTime() : Date.now());
                        const alreadyCollected = collections.chat.some(c => c.content === content && c.originalTime === ts);
                        if (!alreadyCollected && Math.random() < CHAT_HISTORY_CHANCE) {
                            ...
                            addCollection('chat', content, ts, extra);
        ...
        Promise.all(tasks).then(function() { saveCollections(); });
    }
    ...
    function renderGiftCabinet() {
        ...
        if (!gifts || gifts.length === 0) {
            try {
                const saved = localStorage.getItem((window.APP_PREFIX || '') + 'shop_gift_cabinet');
        ...
        listEl.innerHTML = gifts.map((item, idx) => {
            const replyPreview = item.replies && item.replies.length > 0
                ? `<div style="margin-top:8px;padding:8px;background:rgba(233,69,96,0.08);border-radius:8px;font-size:0.78rem;color:#c0392b;line-height:1.4;">${escapeHtml(item.replies[0].text)}</div>`
                : '';
            return `
                <div class="ta-phone-item">
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
                        <span style="font-size:1.5rem;">${escapeHtml(item.icon) || '📦'}</span>
                        <div style="flex:1;">
