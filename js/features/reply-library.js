...
// 分组颜色白名单：只允许十六进制 / rgb(a) / hsl(a) / 颜色名，阻断 " / ` / < 等可突破 style 属性的字符
function _rlSafeColor(c) {
    if (typeof c !== 'string') return '#868E96';
...
function _renderModernToolbar() {
            ...
            ">
                ...
                ${ctx.groups.map(g => {
                    ...
                    return `<button class="gfp-btn ${_activeGroupFilter === g.id ? 'gfp-active' : ''} ${g.disabled ? 'gfp-disabled' : ''}"
                        data-filter="${g.id}"
                        style="${_activeGroupFilter === g.id ? `background:${_rlSafeColor(g.color)}22;border-color:${_rlSafeColor(g.color)};color:${_rlSafeColor(g.color)};` : ''}">
                        <span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:${_rlSafeColor(g.color)};margin-right:4px;flex-shrink:0;vertical-align:middle;"></span>
                        ${String(g.name == null ? '' : g.name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')} <span class="gfp-count">${cnt}</span>
    ...
    toolbar.querySelectorAll('.gfp-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            _activeGroupFilter = f === 'all' ? null : (f === 'ungrouped' ? 'ungrouped' : (isFinite(parsed) ? parsed : null));
            renderReplyLibrary();
        });
    });
...
function _renderGroupBlock(list, group, groupItems, disabledSet, isUngrouped = false) {
    ...
    const colorDot = _rlSafeColor(group.color);
    ...
    section.innerHTML = `
        <div class="rl-group-header${isCollapsed ? ' collapsed' : ''}" id="grp-hdr-${group.id}" style="${isDisabled ? 'opacity:0.5;' : ''}">
            <div class="rl-group-tag" id="grp-tag-${group.id}" title="${isDisabled ? '点击启用此分组' : '点击屏蔽此分组'}">
                ...
                <span style="font-size:12px;font-weight:700;color:${colorDot};">${String(group.name == null ? '' : group.name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span>
...
function _createCard(item, index, disabledSet) {
    ...
    const groupBadge = (() => {
        ...
        return `<span style="
            ...
            background:${_rlSafeColor(g.color)}18;color:${_rlSafeColor(g.color)};border:1px solid ${_rlSafeColor(g.color)}30;
            margin-top:5px;flex-shrink:0;
        ">
            <span style="width:5px;height:5px;border-radius:50%;background:${_rlSafeColor(g.color)};flex-shrink:0;"></span>
            ${String(g.name == null ? '' : g.name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}
        </span>`;
    })();

    const itemParts = item.split('|');
    const displayText = itemParts.length > 1
        ? `<span style="font-size:13px;">${String(itemParts[0]).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span><span style="font-size:11px;opacity:0.6;display:block;margin-top:1px;">${String(itemParts[1]).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span>`
        : `<span style="font-size:13px;">${String(item).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span>`;
...
function _renderAtmosphereList(list, items) {
    ...
    items.forEach((item, idx) => {
        ...
        div.innerHTML = `
            <span class="custom-reply-text">${String(item).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#039;').replace('|', '<br><small style="opacity:.65">')}</span>
        ...
        panel.querySelector('#gm-list').innerHTML = noGroups
            ...
            : groups.map((g, i) => `
                <div style="
                    ...
                    transition:all 0.15s;
                ">
                    <span style="width:12px;height:12px;border-radius:50%;background:${_rlSafeColor(g.color)};flex-shrink:0;box-shadow:0 0 0 2px ${_rlSafeColor(g.color)}30;"></span>
...
function _showSingleItemGroupPicker(itemText, ctx) {
    ...
    panel.innerHTML = `
        ...
        <div style="display:flex;flex-direction:column;gap:7px;max-height:55vh;overflow-y:auto;margin-bottom:14px;">
            ...
            ${groups.map((g, i) => `
                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:10px 12px;border-radius:11px;border:1.5px solid ${currentGroup?.id === g.id ? _rlSafeColor(g.color) : 'var(--border-color)'};background:${currentGroup?.id === g.id ? _rlSafeColor(g.color) + '10' : 'var(--primary-bg)'};">
                    <input type="radio" name="sgp" value="${i}" ${currentGroup?.id === g.id ? 'checked' : ''} style="accent-color:${_rlSafeColor(g.color)};">
                    <span style="width:9px;height:9px;border-radius:50%;background:${_rlSafeColor(g.color)};flex-shrink:0;"></span>
                    <span style="flex:1;font-size:13px;color:var(--text-primary);font-weight:600;">${g.name}</span>
...
function _showBatchGroupPicker() {
    ...
    panel.innerHTML = `
        ...
        <div style="display:flex;flex-direction:column;gap:7px;max-height:50vh;overflow-y:auto;margin-bottom:14px;">
            ...
            ${groups.map((g, i) => `
                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:10px 12px;border-radius:11px;border:1.5px solid var(--border-color);background:var(--primary-bg);">
                    <input type="radio" name="bgp" value="${i}" style="accent-color:${_rlSafeColor(g.color)};">
                    <span style="width:9px;height:9px;border-radius:50%;background:${_rlSafeColor(g.color)};flex-shrink:0;"></span>
                    <span style="flex:1;font-size:13px;color:var(--text-primary);font-weight:600;">${g.name}</span>
                    <span style="font-size:11px;color:var(--text-secondary);">${(g.items||[]).length} 条</span>
                </label>
...
function _showGroupExportPicker(type) {
    ...
    cfg.groups.forEach((g, i) => {
        ...
        row.innerHTML = `
            <input type="checkbox" value="${i}" style="width:16px;height:16px;accent-color:${_rlSafeColor(g.color)};flex-shrink:0;" checked>
            <span style="width:10px;height:10px;border-radius:50%;background:${_rlSafeColor(g.color)};flex-shrink:0;"></span>
        ...
        try {
            ...
            if (overwrite) {
                selected.forEach(m => {
                    ...
                    else if (m.key === 'customReplyGroups')  { window.customReplyGroups  = ensureReplyGroupIds(data.customReplyGroups); }
                ...
                selected.forEach(m => {
                    if (m.key === 'customReplies') {
                        ...
                        data.customReplyGroups.forEach(dg => {
                            if (!customReplyGroups.find(g => g.name === dg.name)) {
                                if (typeof dg.id !== 'number' || !isFinite(dg.id) || (dg.id || 0) <= 0) dg.id = Date.now() + customReplyGroups.length;
                                customReplyGroups.push(dg);
                            }
                        });
...
function _showBatchAddDialog() {
    ...
    const groupPillsHTML = hasGroups ? `
        ...
        <button class="ba-grp-pill" data-gidx="${i}" style="
            padding:5px 13px;border-radius:20px;font-size:12px;font-family:var(--font-family);cursor:pointer;
            border:1.5px solid ${_rlSafeColor(g.color)}44;background:${_rlSafeColor(g.color)}18;color:${_rlSafeColor(g.color)};font-weight:600;
            flex-shrink:0;transition:all .15s;
        ">
            <span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:${_rlSafeColor(g.color)};margin-right:4px;vertical-align:middle;"></span>${String(g.name == null ? '' : g.name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}
        </button>`).join('')}
    ` : '';
