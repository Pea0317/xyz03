document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        try { document.dispatchEvent(new CustomEvent('app-data-ready')); } catch (e) {}

        // 保活：补发离开期间对方发来的消息（自动发送开启时）
        setTimeout(() => {
            try { if (typeof window._catchUpMissedMessages === 'function') window._catchUpMissedMessages(); } catch (e) { console.warn('[keepalive] catch-up error:', e); }
        }, 600);
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.visibilityState === 'hidden') {
                try { if (typeof window._recordLastActive === 'function') window._recordLastActive(); } catch (e) {}
                try {
                ...
                try {
                    // 只有启动流程的数据加载完成后才允许自动恢复备份，
                    // 否则强制刷新时 IndexedDB 尚未读回（messages 为空），
                    // 备份里哪怕只有1条旧消息都会触发恢复，进而把真实数据覆盖掉
                    if (window.__appDataReady !== true) return;
        ...
        // 保活心跳：前台时定期刷新「最后活跃时间」，确保离开时长计算准确
        setInterval(() => {
            try { if (!document.hidden && typeof window._recordLastActive === 'function') window._recordLastActive(); } catch (e) {}
        }, 60 * 1000);
...
window.addEventListener('load', function() {
    ...
    setTimeout(function() {
        // 早安弹窗必须在"数据加载完成(含 moodData/对方今日心情)"之后再构建，
        // 否则 window.moodData 还是空的，每天第一次打开都只会显示「对方还没有记录」，
        // 且当天再也不会重试（dailyGreetingShown 已经被写掉）。
        // 这里最多等 20 秒（50 次 × 400ms），到点后即便还差一点也照常构建，不无限阻塞。
        var _greetWait = 0;
        (function tryGreeting() {
            if (window.__appDataReady !== true && _greetWait < 50) {
                _greetWait++;
                setTimeout(tryGreeting, 400);
                return;
            }
            try {
