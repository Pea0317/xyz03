...
function renderAnniversariesList() {
    ...
    anniversaries.forEach(ann => {
        ...
        const html = `
            <div class="ann-item-card ${typeClass}" data-ann-id="${escAttr(ann.id)}" onclick="selectAnnCard(${(/^\d+$/.test(ann.id) ? ann.id : (ann.id >= 0 ? Number(ann.id) : 0))})" style="cursor:pointer;" title="起始日：${escAttr(ann.date)}">
                ...
                <div style="display:flex; align-items:center;">
                    ...
                    <div class="ann-delete-btn" onclick="event.stopPropagation(); deleteAnniversaryItem(${(/^\d+$/.test(ann.id) ? ann.id : (ann.id >= 0 ? Number(ann.id) : 0))})">
