...
function showEmojiTab() {
    ...
    customEmojis.forEach(emoji => {
        const item = document.createElement('div');
        item.className = 'picker-item';
        item.innerHTML = `<span style="font-size:24px;">${String(emoji == null ? '' : emoji).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span>`;
        item.onclick = () => {
            const input = document.getElementById('message-input');
            input.value += emoji;
            document.getElementById('user-sticker-picker').classList.remove('active');
...
function _buildDailyGreeting() {
    try {
        ...
        // 启动时序兜底：如果今天还没有对方心情（moodData 已就绪但因时序漏抽），就地补抽一次
        if ((!todayMood || !todayMood.partner) && typeof checkPartnerDailyMood === 'function') {
            try { checkPartnerDailyMood(); moodDataRaw = window.moodData || {}; todayMood = moodDataRaw[todayStr]; } catch(e) {}
        }
...
window.renderAnnStatusPool = function() {
    ...
    pool.forEach(function(item, idx) {
        ...
        var iconHtml = item.iconImg
            ? '<img src="' + escAttr(item.iconImg) + '" style="width:26px;height:26px;border-radius:50%;object-fit:cover;flex-shrink:0;">'
            : '<span style="font-size:18px;min-width:26px;text-align:center;flex-shrink:0;">' + escapeHtml(item.icon || '✦') + '</span>';
        row.innerHTML = iconHtml
            + '<div style="flex:1;min-width:0;">'
            + '<div style="color:var(--text-primary);font-weight:500;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escapeHtml(item.status || '—') + '</div>'
            + (item.label ? '<div style="color:var(--accent-color);font-size:10px;letter-spacing:1.5px;margin-top:2px;opacity:0.8;">' + escapeHtml(item.label) + '</div>' : '')
            + '</div>'
