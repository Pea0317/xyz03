...
window._refreshMoodDisplays = function() {
    try { if (typeof _buildDailyGreeting === 'function') _buildDailyGreeting(); } catch(e) {}
    try {
        var overlay = document.getElementById('mood-selector-overlay');
        if (overlay && overlay.classList.contains('active') && window.selectedDateStr) {
            var detailData = (window.moodData || {})[window.selectedDateStr];
            if (detailData && typeof showDayDetails === 'function') {
                showDayDetails(window.selectedDateStr, detailData);
            }
        }
    } catch(e) {}
};
