...
function initComboMenu() {
    ...
    function renderUserPokeMenu() {
        ...
        // 拍一拍字卡库：直接展示库内全部拍一拍字卡（去重快捷动作与已分组条目）
        const quickTexts = new Set(userPresets.map(x => String(x)));
        const cardItems = [];
        ...
        pokeGroups.forEach(function(g) {
            ...
            g.items.forEach(function(it) {
                const key = String(it);
                if (quickTexts.has(key) || cardSeen.has(key)) return;
                cardSeen.add(key);
                cardItems.push(it);
            });
        });
        ...
        if (cardItems.length > 0) {
            ...
            libTitle.style.cssText = 'font-size:12px;color:var(--text-secondary);margin:8px 0 5px;';
            libTitle.innerText = '拍一拍字卡库';
            wrapper.appendChild(libTitle);

            cardItems.forEach((cardText) => {
                const item = document.createElement('div');
                item.className = 'poke-quick-item';
                item.style.cssText = 'background:var(--secondary-bg);box-shadow:none;';
                item.innerText = cardText;
                item.onclick = (e) => {
                    e.stopPropagation();
                    let sendText = cardText;
                    if (typeof window._sanitizePokeTextForDisplay === 'function') {
                        sendText = window._sanitizePokeTextForDisplay(sendText);
                    }
                    addMessage({
                        id: Date.now(),
                        text: _formatPokeText(`${settings.myName} ${sendText}`),
                        timestamp: new Date(),
                        type: 'system'
                    });
                    picker.classList.remove('active');
            ...
            const libEmpty = document.createElement('div');
            libEmpty.style.cssText = 'text-align:center;padding:14px 10px;color:var(--text-secondary);font-size:12px;';
            libEmpty.innerHTML = '<i class="fas fa-layer-group" style="font-size:16px;margin-bottom:5px;display:block;opacity:0.4;"></i>字卡库暂无内容<br><span style="font-size:11px;opacity:0.7;">可在 氛围感 → 拍一拍 中添加</span>';
            wrapper.appendChild(libEmpty);
        }
