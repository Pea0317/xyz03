function renderStatsContent() {
        ...
        function renderSessionList() {
            ...
            <div class="session-item ${session.id === SESSION_ID ? 'active': ''}" data-id="${escAttr(session.id)}">
            <div class="session-info">
            <div class="session-name">${String(session.name == null ? '' : session.name).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
        ...
        return `
            ...
            ">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
                    ${avatarHtml}
                    <span style="font-size:12px;font-weight:600;color:var(--accent-color);">${String(senderName == null ? '' : senderName).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span>
...
window._runMsgSearch = function() {
    ...
    resultsEl.innerHTML = results.slice(0, 100).map(msg => {
        ...
        const content = msg.text
            ...
            : (msg.image ? `<img src="${escAttr(msg.image)}" style="max-height:60px;border-radius:6px;display:block;margin-top:4px;" loading="lazy">` : '');
...
function initComboMenu() {
    ...
    function updateAddBtnVisibility(tabId) {
        ...
        if (manageBtn) manageBtn.style.display = (tabId === 'my-sticker') ? 'flex' : 'none';
    }
    ...
    function switchTab(tabId) {
        ...
        if (tabId === 'my-sticker') {
            ...
            // 表情面板里的"问卷"：直接打开高级功能那套问卷页面（聊天上下文入口，
            // 从这里发出的问卷会走"聊天节奏回答 + 回答发到聊天里"的链路）
            if (typeof window._surveyOpenListModalChat === 'function') {
                window._surveyOpenListModalChat();
            } else if (typeof window._surveyOpenListModal === 'function') {
                window._surveyOpenListModal();
            }
        } else {
            renderUserPokeMenu();
        }
    ...
    function _myStickerOpenManageModal(groupId) {
        ...
        var groupNameEsc = String(groupName == null ? '' : groupName).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
        var modalTitle = isDefault ? '管理' + groupName : '管理' + groupName + '分组';
        var modalTitleEsc = String(modalTitle == null ? '' : modalTitle).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&#39;');
        ...
        var groupActionsHtml = isDefault
            ...
            : (
                '<div class="my-sticker-manage-group-row" style="margin-bottom:14px;">' +
                    '<span class="my-sticker-manage-group-name" id="my-sticker-manage-name-display">' + groupNameEsc + '</span>' +
        ...
        modal.innerHTML =
            '<div class="modal-content" style="max-width:360px;max-height:85vh;">' +
                '<div class="modal-title"><i class="fas fa-cog"></i><span>' + modalTitleEsc + '</span></div>' +
                groupActionsHtml +
    ...
    function renderUserPokeMenu() {
        ...
        // 拍一拍字卡库：分组 + 库内自定义拍一拍去重后全部展示；库里为空时用内置常用动作兜底
        const DEFAULT_POKE_CARDS = [
            '拍了拍对方的头',
            '戳了戳对方的脸颊',
            '抱住了对方',
            '给对方比了个心',
        ...
        const pokeGroups = (typeof window.customPokeGroups !== 'undefined' && Array.isArray(window.customPokeGroups)) ? window.customPokeGroups : [];
        const allPokes = (typeof customPokes !== 'undefined' && Array.isArray(customPokes)) ? customPokes : [];

        const cardItems = [];
        const cardSeen = new Set();
        pokeGroups.forEach(function (g) {
            if (!g || g.disabled || !Array.isArray(g.items)) return;
            g.items.forEach(function (it) {
                const key = String(it);
                if (cardSeen.has(key)) return;
                cardSeen.add(key);
                cardItems.push(it);
            });
        });
        allPokes.forEach(function (it) {
            const key = String(it);
            if (cardSeen.has(key)) return;
            cardSeen.add(key);
            cardItems.push(it);
        });
        if (cardItems.length === 0) cardItems.push.apply(cardItems, DEFAULT_POKE_CARDS);

        const title = document.createElement('div');
        title.className = 'poke-section-title';
        title.innerHTML = '<i class="fas fa-layer-group"></i> 拍一拍字卡库';
        wrapper.appendChild(title);

        const grid = document.createElement('div');
        grid.className = 'poke-library-grid';
        const myName = (typeof settings !== 'undefined' && settings.myName) ? settings.myName : '我';
        cardItems.forEach(function (cardText) {
            let cleanText = cardText;
            if (typeof window._sanitizePokeTextForDisplay === 'function') {
                cleanText = window._sanitizePokeTextForDisplay(cleanText);
            }
            if (!cleanText) cleanText = '拍了拍对方';
            const item = document.createElement('div');
            item.className = 'poke-card';
            item.textContent = cleanText;
            item.title = cleanText;
            item.onclick = (e) => {
                e.stopPropagation();
                // 已保存的完整句子（以我的名字开头）直接发送，否则按"动作片段"补上名字
                const sendText = cleanText.indexOf(myName) === 0 ? cleanText : `${myName} ${cleanText}`;
                addMessage({
                    id: Date.now(),
                    text: _formatPokeText(sendText),
                    timestamp: new Date(),
                    type: 'system'
                });
        ...
        function renderRank(words) {
            ...
            el.innerHTML = words.slice(0, 10).map(function(item, i) {
                ...
                return '<div class="wc-rank-item">'
                    ...
                    + '<span class="wc-rank-word">' + String(item.word == null ? '' : item.word).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</span>'
